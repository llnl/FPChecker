       ...
       if (trunc_factor > 0)
       {
          ...
          for (i = start; i < stop; i++)
          {
            ...
            for (j = P_diag_i[i]; j < P_diag_i[i+1]; j++)
               max_coef = (max_coef < hypre_fabs(P_diag_data[j])) ? 
                      hypre_fabs(P_diag_data[j]) : max_coef;
            for (j = P_offd_i[i]; j < P_offd_i[i+1]; j++)
               max_coef = (max_coef < hypre_fabs(P_offd_data[j])) ? 
                      hypre_fabs(P_offd_data[j]) : max_coef;
            ...
            for (j = start_j; j < P_diag_i[i+1]; j++)
            {
               ...
               if (hypre_fabs(P_diag_data[now_checking]) < max_coef)
               {
                  num_lost++;
                  now_checking++;
               }
            ...
            for (j = start_j; j < P_offd_i[i+1]; j++)
            {
               ...
               if (hypre_fabs(P_offd_data[now_checking_offd]) < max_coef)
               {
                  num_lost_offd++;
                  now_checking_offd++;
               }
                                   ...
                                   HYPRE_Real w)
{
   ...
   for (i=0; i < num_rows; i++)
   {
      ...
      if (w == 0)
      {
         w_local = hypre_fabs(A_diag_data[index]);
         for (j = index+1; j < A_diag_i[i+1]; j++)
            w_local += hypre_fabs(A_diag_data[j]);
         for (j = A_offd_i[i]; j < A_offd_i[i+1]; j++)
            w_local += hypre_fabs(A_offd_data[j]);
         invdiag = -1/w_local;
