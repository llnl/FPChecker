...
hypre_ParCSRMatrixScaledNorm( hypre_ParCSRMatrix *A, HYPRE_Real *scnorm)
{
   ...
   {
      dis_data[i] = 1.0/hypre_sqrt(hypre_fabs(diag_data[diag_i[i]]));
   }
   ...
   for (i=0; i < num_rows; i++)
   {
      ...
      {
	 sum_data[i] += hypre_fabs(diag_data[j])*dis_data[i]*dis_data[diag_j[j]];
      }
   }   
   ...
   for (i=0; i < num_rows; i++)
   {
      ...
      {
	 sum_data[i] += hypre_fabs(offd_data[j])*dis_data[i]*dis_ext_data[offd_j[j]];
      }
   }   
