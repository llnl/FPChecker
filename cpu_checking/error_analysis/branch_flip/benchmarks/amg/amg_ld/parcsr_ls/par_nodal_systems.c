         ...
         for (i=0; i < num_nodes; i++)
         {
	       ...
	       for (k=A_diag_i[row]; k < A_diag_i[row+1]; k++)
	       {
	          ...
	          if (counter[k_map] < start_index)
	          {
	             counter[k_map] = index;
	             AN_diag_j[index] = k_map;
	             AN_diag_data[index] = hypre_fabs(A_diag_data[k]);
	             index++;
	          }
	          else
	          {
	             AN_diag_data[counter[k_map]] += hypre_fabs(A_diag_data[k]);
	          }
	       }
	       row++;
            }
         ...
         for (i=0; i < num_nodes; i++)
         {
            for (j=0; j < num_functions; j++)
            {
      	       for (k=A_diag_i[row]; k < A_diag_i[row+1]; k++)
      	       {
      	          ...
      	          {
      	             if (hypre_fabs(A_diag_data[k]) > 
				hypre_fabs(AN_diag_data[counter[k_map]]))
         ...
         for (i=0; i < num_nodes; i++)
         {
	       ...
	       for (k=A_diag_i[row]; k < A_diag_i[row+1]; k++)
	       {
	          ...
	          if (counter[k_map] < start_index)
	          {
	             counter[k_map] = index;
	             AN_diag_j[index] = k_map;
	             data[index*num_functions + j] = hypre_fabs(A_diag_data[k]);
	             index++;
	          }
	          else
	          {
	             data[(counter[k_map])*num_functions + j] += hypre_fabs(A_diag_data[k]);
	          }
	       }
	       row++;
            }
            ...
            for (i=0; i < num_nodes; i++)
            {
	          ...
	          for (k=A_offd_i[row]; k < A_offd_i[row+1]; k++)
	          {
	             ...
	             if (counter[k_map] < start_index)
	             {
	                counter[k_map] = index;
	                AN_offd_j[index] = k_map;
	                AN_offd_data[index] = hypre_fabs(A_offd_data[k]);
	                index++;
	             }
	             else
	             {
	                AN_offd_data[counter[k_map]] += hypre_fabs(A_offd_data[k]);
	             }
	          }
	          row++;
               }
            ...
            for (i=0; i < num_nodes; i++)
            {
               for (j=0; j < num_functions; j++)
               {
      	          for (k=A_offd_i[row]; k < A_offd_i[row+1]; k++)
      	          {
      	             ...
      	             {
      	                if (hypre_fabs(A_offd_data[k]) > 
				hypre_fabs(AN_offd_data[counter[k_map]]))
               ...
               for (j=0; j < num_functions; j++)
               {
                  for (k=A_offd_i[row]; k < A_offd_i[row+1]; k++)
                  {
                     ...
                     if (counter[k_map] < start_index)
                     {
                        counter[k_map] = index;
                        AN_offd_j[index] = k_map;
                        data[index*num_functions + j] = hypre_fabs(A_offd_data[k]);
                        index++;
                     }
                     else
                     {
                        data[(counter[k_map])*num_functions + j] += hypre_fabs(A_offd_data[k]);
                     }
                  }
                  row++;
               }
