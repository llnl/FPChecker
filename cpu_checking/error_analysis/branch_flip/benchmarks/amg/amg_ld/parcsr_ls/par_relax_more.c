                               ...
                               HYPRE_Real *max_eig)
{
   ...
   for ( i = 0; i < A_num_rows; i++ )
   {
      ...
      {
         row_sum += hypre_fabs(A_diag_data[j]);
      }
      for (j = A_offd_i[i]; j < A_offd_i[i+1]; j++)
      {
         row_sum += hypre_fabs(A_offd_data[j]);
      }
      if (scale)
      {
         if (diag_value != 0.0)
   ...
   if (variant == 1 )
   {
                                ...
                                one less that  resid poly: r(t) = 1 - t*s(t) */ 
      {
         ...
         case 3: /* -(6*del^2*th - 12*del*th^2 - t^2*(4*del + 16*th) + t*(12*del*th - 3*del^2 + 24*th^2) + 3*del^3 + 4*t^3 - 16*th^3)/(4*del*th^3 - 3*del^2*th^2 - 3*del^3*th + 4*th^4)*/
            den = - (4*delta*hypre_pow(theta,3) - 3*hypre_pow(delta,2)*hypre_pow(theta,2) - 3*hypre_pow(delta,3)*theta + 4*hypre_pow(theta,4) );
            
            coefs[0] = (6*hypre_pow(delta,2)*theta - 12*delta*hypre_pow(theta,2) + 3*hypre_pow(delta,3) - 16*hypre_pow(theta,3)   )/den;
...
*/
    ...
    *ierr = 0;
    ...
    for (l = 1; l <= i__1; ++l) {
        j = 0;
        h = (d__1 = d[l],hypre_fabs(d__1)) + (d__2 = e[l],hypre_fabs(d__2));
        if (tst1 < h) {
            tst1 = h;
        }
/*     .......... LOOK FOR SMALL SUB-DIAGONAL ELEMENT .......... */
        i__2 = *n;
        for (m = l; m <= i__2; ++m) {
            tst2 = tst1 + (d__1 = e[m],hypre_fabs(d__1));
            if (tst2 == tst1) {
                goto L120;
            }
...
/*     .......... FOR I=M-1 STEP -1 UNTIL L DO -- .......... */
        ...
        e[l] = s * p;
        d[l] = c * p;
        tst2 = tst1 + (d__1 = e[l],hypre_fabs(d__1));
        if (tst2 > tst1) {
            goto L130;
        }
L210:
...
/* Computing MAX */
    d__1 = hypre_fabs(*a),d__2 = hypre_fabs(*b);
    p = hypre_max(d__1,d__2);
    if (!p) {
        goto L20;
    }
/* Computing MIN */
    d__2 = hypre_fabs(*a),d__3 = hypre_fabs(*b);
/* Computing 2nd power */
