...
HYPRE_Int hypre_ParCSRMatrixFixZeroRows(hypre_ParCSRMatrix *A)
{
   ...
   for (i = 0; i < num_rows; i++)
   {
      l1_norm = 0.0;
      for (j = A_diag_I[i]; j < A_diag_I[i+1]; j++)
         l1_norm += hypre_fabs(A_diag_data[j]);
      if (num_cols_offd)
         for (j = A_offd_I[i]; j < A_offd_I[i+1]; j++)
            l1_norm += hypre_fabs(A_offd_data[j]);
                                     ...
                                     HYPRE_Real **l1_norm_ptr)
{
   ...
   if (option == 1)
   {
      for (i = 0; i < num_rows; i++)
      {
         ...
         if (cf_marker == NULL)
         {
            ...
            for (j = A_diag_I[i]; j < A_diag_I[i+1]; j++)
               l1_norm[i] += hypre_fabs(A_diag_data[j]);
            /* Add the l1 norm of the offd part of the ith row */
            if (num_cols_offd)
            {
               for (j = A_offd_I[i]; j < A_offd_I[i+1]; j++)
                  l1_norm[i] += hypre_fabs(A_offd_data[j]);
            }
         }
         else
         {
   ...
   else if (option == 2)
   {
      for (i = 0; i < num_rows; i++)
      {
         ...
         l1_norm[i] = hypre_fabs(A_diag_data[A_diag_I[i]]);
         if (cf_marker == NULL)
         {
            /* Add the l1 norm of the offd part of the ith row */
            if (num_cols_offd)
   ...
   else if (option == 4)
   {
      ...
      {
         /* Add the diag element of the ith row */
         diag = l1_norm[i] = hypre_fabs(A_diag_data[A_diag_I[i]]);
         if (cf_marker == NULL)
         {
            ...
            {
               for (j = A_offd_I[i]; j < A_offd_I[i+1]; j++)
                  l1_norm[i] += 0.5*hypre_fabs(A_offd_data[j]);
            }
         }
         else
         {
   ...
   /* Compute Pi = [Pi_x, Pi_y, Pi_z] */
   {
      ...
      /* Fill-in the diagonal part */
      {
         ...
         for (i = 0; i < G_diag_nrows; i++)
            ...
            {
               *Pi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gx_data[i];
               *Pi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gy_data[i];
               if (dim == 3)
                  *Pi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gz_data[i];
            }
      }
      ...
      /* Fill-in the off-diagonal part */
      {
         ...
         for (i = 0; i < G_offd_nrows; i++)
            ...
            {
               *Pi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gx_data[i];
               *Pi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gy_data[i];
               if (dim == 3)
                  *Pi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gz_data[i];
            }
   ...
   /* Compute Pix, Piy, Piz  */
   {
      ...
      if (dim == 3)
      {
         ...
         for (i = 0; i < G_diag_nrows; i++)
            for (j = G_diag_I[i]; j < G_diag_I[i+1]; j++)
            {
               *Pix_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gx_data[i];
               *Piy_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gy_data[i];
               *Piz_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gz_data[i];
            }
      }
      else
      {
      ...
      if (dim == 3)
      {
         ...
         for (i = 0; i < G_offd_nrows; i++)
            for (j = G_offd_I[i]; j < G_offd_I[i+1]; j++)
            {
               *Pix_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gx_data[i];
               *Piy_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gy_data[i];
               *Piz_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gz_data[i];
            }
      ...
      /* Fill-in the diagonal part */
      {
         ...
         for (i = 0; i < G_diag_nrows; i++)
            for (j = G_diag_I[i]; j < G_diag_I[i+1]; j++)
            {
               ...
               *GPi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gx_data[i];
               *GPi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gy_data[i];
               if (dim == 4)
                  *GPi_diag_data++ = hypre_fabs(G_diag_data[j]) * 0.5 * Gz_data[i];
            }
      }
      ...
      /* Fill-in the off-diagonal part */
      {
         ...
         for (i = 0; i < G_offd_nrows; i++)
            for (j = G_offd_I[i]; j < G_offd_I[i+1]; j++)
            {
               ...
               *GPi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gx_data[i];
               *GPi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gy_data[i];
               if (dim == 4)
                  *GPi_offd_data++ = hypre_fabs(G_offd_data[j]) * 0.5 * Gz_data[i];
            }
   ...
   if (ams_data -> interior_nodes)
   {
         ...
         the sparsity pattern of A is subset of the sparsity pattern of G0 G0^T */
      {
         ...
         /* scale (penalize) G0 G0^T before adding it to the matrix */
         {
            ...
            lfactor = -1;
            for (i = 0; i < nnzB; i++)
               if (hypre_fabs(dataB[i]) > lfactor)
                  lfactor = hypre_fabs(dataB[i]);
            ...
            if (cf_marker == NULL)
            {
               ...
               for (j = A_diag_I[i]; j < A_diag_I[i+1]; j++)
               {
                  ...
                  if (ii == i || ii < ns || ii >= ne)
                  {
                     if (ii == i)
                     {
                        diag = hypre_fabs(A_diag_data[j]);
                        l1_norm[i] += hypre_fabs(A_diag_data[j]);
                     }
                     else
                        l1_norm[i] += 0.5*hypre_fabs(A_diag_data[j]);
                  }
               }
               /* Add the l1 norm of the offd part of the ith row */
               if (num_cols_offd)
