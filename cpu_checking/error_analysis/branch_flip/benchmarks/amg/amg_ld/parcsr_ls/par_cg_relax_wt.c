			  ...
			  HYPRE_Real 	    *rlx_wt_ptr)
{
   ...
   for (jj = 0; jj < num_cg_sweeps; jj++)
   {
      ...
      row_sum = hypre_fabs(tridiag[jj]) + hypre_fabs(trioffd[jj]);
      if (row_sum > max_row_sum) max_row_sum = row_sum;
      if (jj > 0)
      {
	 row_sum = hypre_fabs(tridiag[jj-1]) + hypre_fabs(trioffd[jj-1])
				+ hypre_fabs(trioffd[jj]);
         if (row_sum > max_row_sum) max_row_sum = row_sum;
	 ...
	 if (hypre_fabs(rlx_wt-rlx_wt_old) < 1.e-3 )
	 {
		...
		HYPRE_Real tol, HYPRE_Int k, HYPRE_Real *ev_ptr)
{
   ...
   while (hypre_fabs(y-z) > tol*(hypre_fabs(y) + hypre_fabs(z)))
   {
      x = (y+z)/2;
