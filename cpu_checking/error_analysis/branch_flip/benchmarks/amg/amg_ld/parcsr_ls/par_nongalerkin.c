                      ...
                      HYPRE_Int  right )
{
    ...
    for (i = left+1; i <= right; i++)
        if (hypre_fabs(w[i]) < hypre_fabs(w[left]))
        {
...
#pragma omp parallel for private(i,diag,row_scale,row_sum,jA) HYPRE_SMP_SCHEDULE
    ...
    for (i = 0; i < num_variables; i++)
    {
        ...
        if ((hypre_fabs(row_sum) > hypre_fabs(diag)*max_row_sum) && (max_row_sum < 1.0))
        {
...
#pragma omp parallel for private(i,j,max_entry,max_entry_offd,global_col,global_row) HYPRE_SMP_SCHEDULE
    ...
    for(i = 0; i < num_variables; i++)
    {
        ...
        {
            if( (RAP_diag_j[j] != i) && (max_entry < hypre_fabs(RAP_diag_data[j]) ) )
            {   max_entry = hypre_fabs(RAP_diag_data[j]); }
        }
        for(j = RAP_offd_i[i]; j < RAP_offd_i[i+1]; j++)
        {
            {
                if( max_entry < hypre_fabs(RAP_offd_data[j]) )
                {   max_entry = hypre_fabs(RAP_offd_data[j]); }
            }
        }
        max_entry *= droptol;
        ...
        for(j = RAP_diag_i[i]; j < RAP_diag_i[i+1]; j++)
        {
            if( hypre_fabs(RAP_diag_data[j]) > max_entry )
            {
...
/*}*/ 
        ...
        for(j = RAP_offd_i[i]; j < RAP_offd_i[i+1]; j++)
        {
            if( hypre_fabs(RAP_offd_data[j]) > max_entry_offd )
            {
...
/*}*/ 
               ...
               else if( (col_indx_RAP < col_indx_Pattern) || has_row_ended)
               {
                   ...
                   if(intersection_len > 0)
                   {
                       ...
                       {   sum_strong_neigh += hypre_fabs(diag_intersection_data[k]); }
                       for(k = 0; k < offd_intersection_len; k++)
                       {   sum_strong_neigh += hypre_fabs(offd_intersection_data[k]); }
                       sum_strong_neigh = RAP_diag_data[j]/sum_strong_neigh;
                       
                       /* When lumping with the diag_intersection, must offset column index */
                       for(k = 0; k < diag_intersection_len; k++)
                       {
                           lump_value = lump_percent * hypre_fabs(diag_intersection_data[k])*sum_strong_neigh;
                           diagonal_lump_value = (1.0 - lump_percent) * hypre_fabs(diag_intersection_data[k])*sum_strong_neigh;
...
/*}*/
                       ...
                       for(k = 0; k < offd_intersection_len; k++)
                       {
                           lump_value = lump_percent * hypre_fabs(offd_intersection_data[k])*sum_strong_neigh;
                           diagonal_lump_value = (1.0 - lump_percent) * hypre_fabs(offd_intersection_data[k])*sum_strong_neigh;
                           neg_lump_value = -1.0 * lump_value;
