...
/* ===== precision switch ===== */
#ifndef WORKING_T
#define WORKING_T double
#endif
typedef WORKING_T working_t;
#if WORKING_T_IS_FLOAT
#  define WSQRT sqrtf
#  define WFABS fabsf
#elif WORKING_T_IS_LD
#  define WSQRT sqrtl
#  define WFABS fabsl
#else
#  define WSQRT sqrt
...
int main(int argc, char **argv) {
    ...
    double Mops, t1, t2, t3, t4, tm, an, tt, gc;
    working_t x1, x2, sx, sy;   /* deviate arithmetic (target precision) */
...
{
    double t1, t2, t3, t4;              /* RNG seed scalars (double) */
    working_t x1, x2, w_t1, w_t2;       /* deviate arithmetic (target precision) */
    int kk, i, ik, l;
...
*/
	...
	for ( i = 0; i < NK; i++) {
            x1 = (working_t)2.0 * (working_t)x[2*i] - (working_t)1.0;
            x2 = (working_t)2.0 * (working_t)x[2*i+1] - (working_t)1.0;
            w_t1 = x1*x1 + x2*x2;                       /* deviate radius^2 (target) */
            if (w_t1 <= (working_t)1.0) {               /* branch in target precision */
		w_t2 = WSQRT((working_t)(-2.0) * (working_t)log((double)w_t1) / w_t1);
		x1 = (x1 * w_t2);			/* Xi (reuse x1) */
		x2 = (x2 * w_t2);			/* Yi (reuse x2) */
		l = max(WFABS(x1), WFABS(x2));
		qq[l] += 1.0;				/* counts */
		sx = sx + x1;				/* sum of Xi */
		sy = sy + x2;				/* sum of Yi */
            }
