/*--------------------------------------------------------------------
  NPB SP (NPB3.0-omp-C) -- standalone single-file FP64 build.
  All physics variables, constants, and math functions are double.
  CLASS = S (grid 12^3, NITER=100, DT=0.015). Serial (no OpenMP).
  Build:  cc -O3 -std=c99 sp.c -lm -o bt_fp64
  NOTE: verify requires NITER=100 (reference table is locked to 100 steps).
--------------------------------------------------------------------*/
#include <stdio.h>
...
/* ---- npbparams.h : NPB SP, CLASS = S (generated) ---- */
...
/* common /fields/ */
static nas_real u       [5][IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1],
...
/* common /work_1d/ */
static nas_real cv[PROBLEM_SIZE], rhon[PROBLEM_SIZE],
...
#pragma omp master
  ...
  if (tmax != 0) {
    mflops = ( NAS_R(881.174) * nas_pow((nas_real)PROBLEM_SIZE, NAS_R(3.0))
	       - NAS_R(4683.91) * pow2((nas_real)PROBLEM_SIZE)
	       + NAS_R(11484.5) * (nas_real)PROBLEM_SIZE
	       - NAS_R(19272.4)) * (nas_real)niter / (tmax*NAS_R(1000000.0));
  } else {
    mflops = NAS_R(0.0);
  }
...
c-------------------------------------------------------------------*/
  for (k = 1; k <= grid_points[2]-2; k++) {
	...
	q[i] = NAS_R(0.5) * (buf[1][i]*ue[1][i] + buf[2][i]*ue[2][i]
		      + buf[3][i]*ue[3][i]);
      }
	...
	forcing[0][i][j][k] = forcing[0][i][j][k] -
	  ...
	  dx1tx1*(ue[0][ip1]-NAS_R(2.0)*ue[0][i]+ue[0][im1]);

	forcing[1][i][j][k] = forcing[1][i][j][k]
	  - tx2 * ((ue[1][ip1]*buf[1][ip1]+c2*(ue[4][ip1]-q[ip1]))-
                   (ue[1][im1]*buf[1][im1]+c2*(ue[4][im1]-q[im1])))+
	  xxcon1*(buf[1][ip1]-NAS_R(2.0)*buf[1][i]+buf[1][im1])+
	  dx2tx1*( ue[1][ip1]-NAS_R(2.0)* ue[1][i]+ue[1][im1]);

	forcing[2][i][j][k] = forcing[2][i][j][k]
	  - tx2 * (ue[2][ip1]*buf[1][ip1]-ue[2][im1]*buf[1][im1])+
	  xxcon2*(buf[2][ip1]-NAS_R(2.0)*buf[2][i]+buf[2][im1])+
	  dx3tx1*( ue[2][ip1]-NAS_R(2.0)*ue[2][i] +ue[2][im1]);
                  
	forcing[3][i][j][k] = forcing[3][i][j][k]
	  - tx2*(ue[3][ip1]*buf[1][ip1]-ue[3][im1]*buf[1][im1])+
	  xxcon2*(buf[3][ip1]-NAS_R(2.0)*buf[3][i]+buf[3][im1])+
	  dx4tx1*( ue[3][ip1]-NAS_R(2.0)* ue[3][i]+ ue[3][im1]);

	forcing[4][i][j][k] = forcing[4][i][j][k]
	  - tx2*(buf[1][ip1]*(c1*ue[4][ip1]-c2*q[ip1])-
		 buf[1][im1]*(c1*ue[4][im1]-c2*q[im1]))+
	  NAS_R(0.5)*xxcon3*(buf[0][ip1]-NAS_R(2.0)*buf[0][i]+
		      buf[0][im1])+
	  xxcon4*(cuf[ip1]-NAS_R(2.0)*cuf[i]+cuf[im1])+
	  xxcon5*(buf[4][ip1]-NAS_R(2.0)*buf[4][i]+buf[4][im1])+
	  dx5tx1*( ue[4][ip1]-NAS_R(2.0)* ue[4][i]+ ue[4][im1]);
      }
...
c-------------------------------------------------------------------*/
	...
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (NAS_R(5.0)*ue[m][i] - NAS_R(4.0)*ue[m][i+1] +ue[m][i+2]);
	i = 2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (-NAS_R(4.0)*ue[m][i-1] + NAS_R(6.0)*ue[m][i] -
 	    NAS_R(4.0)*ue[m][i+1] +     ue[m][i+2]);
      }

      for (m = 0; m < 5; m++) {
	for (i = 3; i <= grid_points[0]-4; i++) {
	  forcing[m][i][j][k] = forcing[m][i][j][k] - dssp*
	    (ue[m][i-2] - NAS_R(4.0)*ue[m][i-1] +
	     NAS_R(6.0)*ue[m][i] - NAS_R(4.0)*ue[m][i+1] + ue[m][i+2]);
	}
      }
	...
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][i-2] - NAS_R(4.0)*ue[m][i-1] +
	   NAS_R(6.0)*ue[m][i] - NAS_R(4.0)*ue[m][i+1]);
	i = grid_points[0]-2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][i-2] - NAS_R(4.0)*ue[m][i-1] + NAS_R(5.0)*ue[m][i]);
      }
    }
  }
...
c-------------------------------------------------------------------*/
  for (k = 1; k <= grid_points[2]-2; k++) {
	...
	buf[0][j] = cuf[j] + buf[1][j] * buf[1][j] + 
	  buf[3][j] * buf[3][j];
	q[j] = NAS_R(0.5)*(buf[1][j]*ue[1][j] + buf[2][j]*ue[2][j] +
		    buf[3][j]*ue[3][j]);
      }
	...
	forcing[0][i][j][k] = forcing[0][i][j][k] -
	  ...
	  dy1ty1*(ue[0][jp1]-NAS_R(2.0)*ue[0][j]+ue[0][jm1]);

	forcing[1][i][j][k] = forcing[1][i][j][k]
	  - ty2*(ue[1][jp1]*buf[2][jp1]-ue[1][jm1]*buf[2][jm1])+
	  yycon2*(buf[1][jp1]-NAS_R(2.0)*buf[1][j]+buf[1][jm1])+
	  dy2ty1*( ue[1][jp1]-NAS_R(2.0)* ue[1][j]+ ue[1][jm1]);

	forcing[2][i][j][k] = forcing[2][i][j][k]
	  - ty2*((ue[2][jp1]*buf[2][jp1]+c2*(ue[4][jp1]-q[jp1]))-
		 (ue[2][jm1]*buf[2][jm1]+c2*(ue[4][jm1]-q[jm1])))+
	  yycon1*(buf[2][jp1]-NAS_R(2.0)*buf[2][j]+buf[2][jm1])+
	  dy3ty1*( ue[2][jp1]-NAS_R(2.0)*ue[2][j] +ue[2][jm1]);

	forcing[3][i][j][k] = forcing[3][i][j][k]
	  - ty2*(ue[3][jp1]*buf[2][jp1]-ue[3][jm1]*buf[2][jm1])+
	  yycon2*(buf[3][jp1]-NAS_R(2.0)*buf[3][j]+buf[3][jm1])+
	  dy4ty1*( ue[3][jp1]-NAS_R(2.0)*ue[3][j]+ ue[3][jm1]);

	forcing[4][i][j][k] = forcing[4][i][j][k]
	  - ty2*(buf[2][jp1]*(c1*ue[4][jp1]-c2*q[jp1])-
		 buf[2][jm1]*(c1*ue[4][jm1]-c2*q[jm1]))+
	  NAS_R(0.5)*yycon3*(buf[0][jp1]-NAS_R(2.0)*buf[0][j]+
		      buf[0][jm1])+
	  yycon4*(cuf[jp1]-NAS_R(2.0)*cuf[j]+cuf[jm1])+
	  yycon5*(buf[4][jp1]-NAS_R(2.0)*buf[4][j]+buf[4][jm1])+
	  dy5ty1*(ue[4][jp1]-NAS_R(2.0)*ue[4][j]+ue[4][jm1]);
      }
...
c-------------------------------------------------------------------*/
	...
	j = 1;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (NAS_R(5.0)*ue[m][j] - NAS_R(4.0)*ue[m][j+1] +ue[m][j+2]);
	j = 2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (-NAS_R(4.0)*ue[m][j-1] + NAS_R(6.0)*ue[m][j] -
	   NAS_R(4.0)*ue[m][j+1] +       ue[m][j+2]);
      }

      for (m = 0; m < 5; m++) {
	for (j = 3; j <= grid_points[1]-4; j++) {
	  forcing[m][i][j][k] = forcing[m][i][j][k] - dssp*
	    (ue[m][j-2] - NAS_R(4.0)*ue[m][j-1] +
	     NAS_R(6.0)*ue[m][j] - NAS_R(4.0)*ue[m][j+1] + ue[m][j+2]);
	}
      }
	...
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][j-2] - NAS_R(4.0)*ue[m][j-1] +
	   NAS_R(6.0)*ue[m][j] - NAS_R(4.0)*ue[m][j+1]);
	j = grid_points[1]-2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][j-2] - NAS_R(4.0)*ue[m][j-1] + NAS_R(5.0)*ue[m][j]);
...
c-------------------------------------------------------------------*/
  for (j = 1; j <= grid_points[1]-2; j++) {
	...
	buf[0][k] = cuf[k] + buf[1][k] * buf[1][k] + 
	  buf[2][k] * buf[2][k];
	q[k] = NAS_R(0.5)*(buf[1][k]*ue[1][k] + buf[2][k]*ue[2][k] +
		    buf[3][k]*ue[3][k]);
      }
	...
	forcing[0][i][j][k] = forcing[0][i][j][k] -
	  ...
	  dz1tz1*(ue[0][kp1]-NAS_R(2.0)*ue[0][k]+ue[0][km1]);

	forcing[1][i][j][k] = forcing[1][i][j][k]
	  - tz2 * (ue[1][kp1]*buf[3][kp1]-ue[1][km1]*buf[3][km1])+
	  zzcon2*(buf[1][kp1]-NAS_R(2.0)*buf[1][k]+buf[1][km1])+
	  dz2tz1*( ue[1][kp1]-NAS_R(2.0)* ue[1][k]+ ue[1][km1]);

	forcing[2][i][j][k] = forcing[2][i][j][k]
	  - tz2 * (ue[2][kp1]*buf[3][kp1]-ue[2][km1]*buf[3][km1])+
	  zzcon2*(buf[2][kp1]-NAS_R(2.0)*buf[2][k]+buf[2][km1])+
	  dz3tz1*(ue[2][kp1]-NAS_R(2.0)*ue[2][k]+ue[2][km1]);

	forcing[3][i][j][k] = forcing[3][i][j][k]
	  - tz2 * ((ue[3][kp1]*buf[3][kp1]+c2*(ue[4][kp1]-q[kp1]))-
		   (ue[3][km1]*buf[3][km1]+c2*(ue[4][km1]-q[km1])))+
	  zzcon1*(buf[3][kp1]-NAS_R(2.0)*buf[3][k]+buf[3][km1])+
	  dz4tz1*( ue[3][kp1]-NAS_R(2.0)*ue[3][k] +ue[3][km1]);

	forcing[4][i][j][k] = forcing[4][i][j][k]
	  - tz2 * (buf[3][kp1]*(c1*ue[4][kp1]-c2*q[kp1])-
		   buf[3][km1]*(c1*ue[4][km1]-c2*q[km1]))+
	  NAS_R(0.5)*zzcon3*(buf[0][kp1]-NAS_R(2.0)*buf[0][k]
		      +buf[0][km1])+
	  zzcon4*(cuf[kp1]-NAS_R(2.0)*cuf[k]+cuf[km1])+
	  zzcon5*(buf[4][kp1]-NAS_R(2.0)*buf[4][k]+buf[4][km1])+
	  dz5tz1*( ue[4][kp1]-NAS_R(2.0)*ue[4][k]+ ue[4][km1]);
      }
...
c-------------------------------------------------------------------*/
	...
	k = 1;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (NAS_R(5.0)*ue[m][k] - NAS_R(4.0)*ue[m][k+1] +ue[m][k+2]);
	k = 2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (-NAS_R(4.0)*ue[m][k-1] + NAS_R(6.0)*ue[m][k] -
	   NAS_R(4.0)*ue[m][k+1] +       ue[m][k+2]);
      }

      for (m = 0; m < 5; m++) {
	for (k = 3; k <= grid_points[2]-4; k++) {
	  forcing[m][i][j][k] = forcing[m][i][j][k] - dssp*
	    (ue[m][k-2] - NAS_R(4.0)*ue[m][k-1] +
	     NAS_R(6.0)*ue[m][k] - NAS_R(4.0)*ue[m][k+1] + ue[m][k+2]);
	}
      }
	...
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][k-2] - NAS_R(4.0)*ue[m][k-1] +
	   NAS_R(6.0)*ue[m][k] - NAS_R(4.0)*ue[m][k+1]);
	k = grid_points[2]-2;
	forcing[m][i][j][k] = forcing[m][i][j][k] - dssp *
	  (ue[m][k-2] - NAS_R(4.0)*ue[m][k-1] + NAS_R(5.0)*ue[m][k]);
      }
    }
  }
...
c-------------------------------------------------------------------*/
  for (m = 0; m < 5; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	for (k = 1; k <= grid_points[2]-2; k++) {
	  forcing[m][i][j][k] = -NAS_R(1.0) * forcing[m][i][j][k];
	}
      }
    }
  }
...
#pragma omp for  
      for (i = 1; i <= grid_points[0]-2; i++) {
	lhs[0][i][j][k] =   NAS_R(0.0);
	lhs[1][i][j][k] = - dttx2 * cv[i-1] - dttx1 * rhon[i-1];
	lhs[2][i][j][k] =   NAS_R(1.0) + c2dttx1 * rhon[i];
	lhs[3][i][j][k] =   dttx2 * cv[i+1] - dttx1 * rhon[i+1];
	lhs[4][i][j][k] =   NAS_R(0.0);
      }
    }
  }
...
#pragma omp for  
	...
	lhs[2][i][j][k] =  NAS_R(1.0) + c2dtty1 * rhoq[j];
...
#pragma omp for  
	...
	lhs[2][i][j][k] =  NAS_R(1.0) + c2dttz1 * rhos[k];
...
--------------------------------------------------------------------*/
  ...
  int i, j, k, m;
  nas_real aux, rho_inv, uijk, up1, um1, vijk, vp1, vm1,
    wijk, wp1, wm1;
...
#pragma omp for
  for (i = 1; i <= grid_points[0]-2; i++) {
	...
	rhs[0][i][j][k] = rhs[0][i][j][k] + dx1tx1 * 
	  (u[0][i+1][j][k] - NAS_R(2.0)*u[0][i][j][k] + 
	   u[0][i-1][j][k]) -
	  tx2 * (u[1][i+1][j][k] - u[1][i-1][j][k]);
	rhs[1][i][j][k] = rhs[1][i][j][k] + dx2tx1 * 
	  (u[1][i+1][j][k] - NAS_R(2.0)*u[1][i][j][k] + 
	   u[1][i-1][j][k]) +
	...
	rhs[2][i][j][k] = rhs[2][i][j][k] + dx3tx1 * 
	  (u[2][i+1][j][k] - NAS_R(2.0)*u[2][i][j][k] +
	   u[2][i-1][j][k]) +
	...
	rhs[3][i][j][k] = rhs[3][i][j][k] + dx4tx1 * 
	  (u[3][i+1][j][k] - NAS_R(2.0)*u[3][i][j][k] +
	   u[3][i-1][j][k]) +
	...
	rhs[4][i][j][k] = rhs[4][i][j][k] + dx5tx1 * 
	  (u[4][i+1][j][k] - NAS_R(2.0)*u[4][i][j][k] +
	   u[4][i-1][j][k]) +
	  ...
	  xxcon5 * (u[4][i+1][j][k]*rho_i[i+1][j][k] - 
		    NAS_R(2.0)*u[4][i][j][k]*rho_i[i][j][k] +
...
#pragma omp for
	...
	rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
	  ( NAS_R(5.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i+1][j][k] +
	    u[m][i+2][j][k]);
      }
    }
  }
...
#pragma omp for
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	  (-NAS_R(4.0)*u[m][i-1][j][k] + NAS_R(6.0)*u[m][i][j][k] -
	   NAS_R(4.0)*u[m][i+1][j][k] + u[m][i+2][j][k]);
      }
    }
  }
...
#pragma omp for
	...
	for (k = 1; k <= grid_points[2]-2; k++) {
	  rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	    (  u[m][i-2][j][k] - NAS_R(4.0)*u[m][i-1][j][k] + 
	       NAS_R(6.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i+1][j][k] + 
	       u[m][i+2][j][k] );
	}
      }
    }
...
#pragma omp for
  for (i = 1; i <= grid_points[0]-2; i++) {
	...
	rhs[0][i][j][k] = rhs[0][i][j][k] + dy1ty1 * 
	  (u[0][i][j+1][k] - NAS_R(2.0)*u[0][i][j][k] + 
	   u[0][i][j-1][k]) -
	  ty2 * (u[2][i][j+1][k] - u[2][i][j-1][k]);
	rhs[1][i][j][k] = rhs[1][i][j][k] + dy2ty1 * 
	  (u[1][i][j+1][k] - NAS_R(2.0)*u[1][i][j][k] + 
	   u[1][i][j-1][k]) +
	...
	rhs[2][i][j][k] = rhs[2][i][j][k] + dy3ty1 * 
	  (u[2][i][j+1][k] - NAS_R(2.0)*u[2][i][j][k] + 
	   u[2][i][j-1][k]) +
	...
	rhs[3][i][j][k] = rhs[3][i][j][k] + dy4ty1 * 
	  (u[3][i][j+1][k] - NAS_R(2.0)*u[3][i][j][k] + 
	   u[3][i][j-1][k]) +
	...
	rhs[4][i][j][k] = rhs[4][i][j][k] + dy5ty1 * 
	  (u[4][i][j+1][k] - NAS_R(2.0)*u[4][i][j][k] + 
	   u[4][i][j-1][k]) +
...
#pragma omp for
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
	  ( NAS_R(5.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i][j+1][k] +
	    u[m][i][j+2][k]);
      }
    }
  }
...
#pragma omp for
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	  (-NAS_R(4.0)*u[m][i][j-1][k] + NAS_R(6.0)*u[m][i][j][k] -
	   NAS_R(4.0)*u[m][i][j+1][k] + u[m][i][j+2][k]);
      }
    }
  }
...
#pragma omp for
	...
	for (k = 1; k <= grid_points[2]-2; k++) {
	  rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	    (  u[m][i][j-2][k] - NAS_R(4.0)*u[m][i][j-1][k] + 
	       NAS_R(6.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i][j+1][k] + 
	       u[m][i][j+2][k] );
	}
      }
    }
...
#pragma omp for
  for (i = 1; i <= grid_points[0]-2; i++) {
	...
	rhs[0][i][j][k] = rhs[0][i][j][k] + dz1tz1 * 
	  (u[0][i][j][k+1] - NAS_R(2.0)*u[0][i][j][k] + 
	   u[0][i][j][k-1]) -
	  tz2 * (u[3][i][j][k+1] - u[3][i][j][k-1]);
	rhs[1][i][j][k] = rhs[1][i][j][k] + dz2tz1 * 
	  (u[1][i][j][k+1] - NAS_R(2.0)*u[1][i][j][k] + 
	   u[1][i][j][k-1]) +
	...
	rhs[2][i][j][k] = rhs[2][i][j][k] + dz3tz1 * 
	  (u[2][i][j][k+1] - NAS_R(2.0)*u[2][i][j][k] + 
	   u[2][i][j][k-1]) +
	...
	rhs[3][i][j][k] = rhs[3][i][j][k] + dz4tz1 * 
	  (u[3][i][j][k+1] - NAS_R(2.0)*u[3][i][j][k] + 
	   u[3][i][j][k-1]) +
	...
	rhs[4][i][j][k] = rhs[4][i][j][k] + dz5tz1 * 
	  (u[4][i][j][k+1] - NAS_R(2.0)*u[4][i][j][k] + 
	   u[4][i][j][k-1]) +
...
#pragma omp for
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
	  ( NAS_R(5.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i][j][k+1] +
	    u[m][i][j][k+2]);
      }
    }
  }
...
#pragma omp for
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	  (-NAS_R(4.0)*u[m][i][j][k-1] + NAS_R(6.0)*u[m][i][j][k] -
	   NAS_R(4.0)*u[m][i][j][k+1] + u[m][i][j][k+2]);
      }
    }
  }
...
#pragma omp for
	...
	for (k = 3*1; k <= grid_points[2]-3*1-1; k++) {
	  rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
	    (  u[m][i][j][k-2] - NAS_R(4.0)*u[m][i][j][k-1] + 
	       NAS_R(6.0)*u[m][i][j][k] - NAS_R(4.0)*u[m][i][j][k+1] + 
	       u[m][i][j][k+2] );
	}
      }
    }
...
--------------------------------------------------------------------*/
  ...
  int i, j, k;
  nas_real t1, t2, t3, ac, ru1, uu, vv, ww, r1, r2, r3, 
    r4, r5, ac2inv;
...
c-------------------------------------------------------------------*/
  ...
  int i, j, k;
  nas_real t1, t2, t3, ac, xvel, yvel, zvel, r1, r2, r3, 
...
--------------------------------------------------------------------*/
    xcrref[0] = NAS_R(2.7470315451339479e-02);
    xcrref[1] = NAS_R(1.0360746705285417e-02);
    xcrref[2] = NAS_R(1.6235745065095532e-02);
    xcrref[3] = NAS_R(1.5840557224455615e-02);
    xcrref[4] = NAS_R(3.4849040609362460e-02);

/*--------------------------------------------------------------------
c    Reference values of RMS-norms of solution error.
--------------------------------------------------------------------*/
    xceref[0] = NAS_R(2.7289258557377227e-05);
    xceref[1] = NAS_R(1.0364446640837285e-05);
    xceref[2] = NAS_R(1.6154798287166471e-05);
    xceref[3] = NAS_R(1.5750704994480102e-05);
    xceref[4] = NAS_R(3.4177666183390531e-05);
...
c    Reference values of RMS-norms of residual.
--------------------------------------------------------------------*/
    xcrref[0] = NAS_R(0.1893253733584e-02);
    xcrref[1] = NAS_R(0.1717075447775e-03);
    xcrref[2] = NAS_R(0.2778153350936e-03);
    xcrref[3] = NAS_R(0.2887475409984e-03);
    xcrref[4] = NAS_R(0.3143611161242e-02);

/*--------------------------------------------------------------------
c    Reference values of RMS-norms of solution error.
--------------------------------------------------------------------*/
    xceref[0] = NAS_R(0.7542088599534e-04);
    xceref[1] = NAS_R(0.6512852253086e-05);
    xceref[2] = NAS_R(0.1049092285688e-04);
    xceref[3] = NAS_R(0.1128838671535e-04);
    xceref[4] = NAS_R(0.1212845639773e-03);
...
c    Reference values of RMS-norms of residual.
--------------------------------------------------------------------*/
    xcrref[0] = NAS_R(2.4799822399300195);
    xcrref[1] = NAS_R(1.1276337964368832);
    xcrref[2] = NAS_R(1.5028977888770491);
    xcrref[3] = NAS_R(1.4217816211695179);
    xcrref[4] = NAS_R(2.1292113035138280);

/*--------------------------------------------------------------------
c    Reference values of RMS-norms of solution error.
--------------------------------------------------------------------*/
    xceref[0] = NAS_R(1.0900140297820550e-04);
    xceref[1] = NAS_R(3.7343951769282091e-05);
    xceref[2] = NAS_R(5.0092785406541633e-05);
    xceref[3] = NAS_R(4.7671093939528255e-05);
    xceref[4] = NAS_R(1.3621613399213001e-04);
...
c    Reference values of RMS-norms of residual.
--------------------------------------------------------------------*/
    xcrref[0] = NAS_R(0.6903293579998e+02);
    xcrref[1] = NAS_R(0.3095134488084e+02);
    xcrref[2] = NAS_R(0.4103336647017e+02);
    xcrref[3] = NAS_R(0.3864769009604e+02);
    xcrref[4] = NAS_R(0.5643482272596e+02);

/*--------------------------------------------------------------------
c    Reference values of RMS-norms of solution error.
--------------------------------------------------------------------*/
    xceref[0] = NAS_R(0.9810006190188e-02);
    xceref[1] = NAS_R(0.1022827905670e-02);
    xceref[2] = NAS_R(0.1720597911692e-02);
    xceref[3] = NAS_R(0.1694479428231e-02);
    xceref[4] = NAS_R(0.1847456263981e-01);
...
c    Reference values of RMS-norms of residual.
--------------------------------------------------------------------*/
    xcrref[0] = NAS_R(0.5881691581829e+03);
    xcrref[1] = NAS_R(0.2454417603569e+03);
    xcrref[2] = NAS_R(0.3293829191851e+03);
    xcrref[3] = NAS_R(0.3081924971891e+03);
    xcrref[4] = NAS_R(0.4597223799176e+03);

/*--------------------------------------------------------------------
c    Reference values of RMS-norms of solution error.
--------------------------------------------------------------------*/
    xceref[0] = NAS_R(0.2598120500183e+00);
    xceref[1] = NAS_R(0.2590888922315e-01);
    xceref[2] = NAS_R(0.5132886416320e-01);
    xceref[3] = NAS_R(0.4806073419454e-01);
    xceref[4] = NAS_R(0.5483377491301e+00);
