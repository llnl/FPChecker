/* NPB CG -- generation-double / solve-fp32. CLASS S.
   makea/sparse/sprnvc/vecset/icnvrt/RNG = double (build identical matrix).
   conj_grad + solver vectors + a[] values + norms = float. */
#include <stdio.h>
...
void timer_clear(int);void timer_start(int);void timer_stop(int);nas_real timer_read(int);
...
/* common /main_flt_mem/ */
static double _sparse_x[NA+2+1];
static double v[NA+1+1];	/* v[1:NA+1] */
static double aelt[NZ+1];	/* aelt[1:NZ] */
static double a[NZ+1];		/* a[1:NZ] gen (double) */
static nas_real a_s[NZ+1];	/* a[1:NZ] solve (target) */
static nas_real x[NA+2+1];	/* x[1:NA+2] */
static nas_real z[NA+2+1];	/* z[1:NA+2] */
static nas_real p[NA+2+1];	/* p[1:NA+2] */
static nas_real q[NA+2+1];	/* q[1:NA+2] */
static nas_real r[NA+2+1];	/* r[1:NA+2] */
...
static void conj_grad (int colidx[], int rowstr[], nas_real x[], nas_real z[],
		       nas_real a[], nas_real p[], nas_real q[], nas_real r[],
		       //double w[],
		       nas_real *rnorm);
...
int main(int argc, char **argv) {
    ...
    nas_real zeta;
    nas_real rnorm;
    nas_real norm_temp11;
    nas_real norm_temp12;
    nas_real t, mflops;
    char class;
    boolean verified;
    nas_real zeta_verify_value, epsilon;
...
c-------------------------------------------------------------------*/
    ...
    { int _k; for(_k=0;_k<=NZ;_k++) a_s[_k]=(nas_real)a[_k]; }
...
c-------------------------------------------------------------------*/
	conj_grad(colidx, rowstr, x, z, a_s, p, q, r,/* w,*/ &rnorm);
...
#pragma omp parallel for default(shared) private(j) reduction(+:norm_temp11,norm_temp12)
	...
	norm_temp12 = 1.0 / nas_sqrt( norm_temp12 );
...
c-------------------------------------------------------------------*/
    ...
    epsilon = 1.0e-10;
    if (class != 'U') {
	if (nas_fabs(zeta - zeta_verify_value) <= epsilon) {
            verified = TRUE;
	    printf(" VERIFICATION SUCCESSFUL\n");
	    printf(" Zeta is    %20.12e\n", NAS_PR(zeta));
	    printf(" Error is   %20.12e\n", NAS_PR(zeta - zeta_verify_value));
	} else {
            verified = FALSE;
	    printf(" VERIFICATION FAILED\n");
	    printf(" Zeta                %20.12e\n", NAS_PR(zeta));
	    printf(" The correct zeta is %20.12e\n", NAS_PR(zeta_verify_value));
	}
    } else {
	verified = FALSE;
...
c---------------------------------------------------------------------*/
{
    ...
    nas_real d, sum, rho, rho0, alpha, beta;
    int i, j, k;
    int cgit, cgitmax = 25;
...
c---------------------------------------------------------------------*/
    sparse(a, colidx, rowstr, n, arow, acol, aelt,
	   firstrow, lastrow, _sparse_x, &(iv[0]), &(iv[n]), nnza);
}
...
#include <sys/time.h>
static nas_real _wt(void){struct timeval tv;static long s=-1;gettimeofday(&tv,(void*)0);if(s<0)s=tv.tv_sec;return (nas_real)(tv.tv_sec-s)+(nas_real)1.0e-6*(nas_real)tv.tv_usec;}
static nas_real _st[64],_el[64];
void timer_clear(int n){_el[n]=(nas_real)0;}
void timer_start(int n){_st[n]=_wt();}
void timer_stop(int n){nas_real x=_wt();_el[n]+=x-_st[n];}
nas_real timer_read(int n){return _el[n];}
void c_print_results(char*nm,char cl,int n1,int n2,int n3,int ni,int nt,nas_real t,nas_real mo,char*ot,int ps,char*ve,char*ct,char*cc,char*cl2,char*cli,char*ci,char*cf,char*clf,char*rd){
