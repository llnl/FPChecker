/* NPB MG -- generation-double / solve-fp64. CLASS S.
   RNG/zran3/power/bubble = double (identical RHS location selection).
   solver (u,r,psinv,resid,rprj3,interp,norm2u3,mg3P) = double.
   v = double RHS (holds only +-1/0, exact); filled by copying double vd. */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
typedef int boolean;
...
c-------------------------------------------------------------------*/
    ...
    { int _a,_b,_c; for(_a=0;_a<m3[lt];_a++)for(_b=0;_b<m2[lt];_b++)for(_c=0;_c<m1[lt];_c++) v[_a][_b][_c]=(double)vd[_a][_b][_c]; }
...
#pragma omp master  
  nthreads = omp_get_num_threads();
    ...
    printf(" Initialization time: %15.3 seconds\n", tinit);
...
/*				0.183103168997d-044 iterations*/
    ...
    c_print_results("MG", Class, nx[lt], ny[lt], nz[lt], 
		    nit, nthreads, t, mflops, "          doubleing point", 
...
#include <sys/time.h>
static double _wt(void){struct timeval tv;static long s=-1;gettimeofday(&tv,(void*)0);if(s<0)s=tv.tv_sec;return (double)(tv.tv_sec-s)+(double)1.0e-6*(double)tv.tv_usec;}
static double _st[64],_el[64];
void timer_clear(int n){_el[n]=(double)0;}
void timer_start(int n){_st[n]=_wt();}
void timer_stop(int n){double x=_wt();_el[n]+=x-_st[n];}
double timer_read(int n){return _el[n];}
void c_print_results(char*nm,char cl,int n1,int n2,int n3,int ni,int nt,double t,double mo,char*ot,int ps,char*ve,char*ct,char*cc,char*cl2,char*cli,char*ci,char*cf,char*clf,char*rd){
