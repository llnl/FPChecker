/* NPB MG -- generation-double / solve-fp32. CLASS S.
   RNG/zran3/power/bubble = double (identical RHS location selection).
   solver (u,r,psinv,resid,rprj3,interp,norm2u3,mg3P) = float.
   v = float RHS (holds only +-1/0, exact); filled by copying double vd. */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
typedef int boolean;
...
void timer_clear(int);void timer_start(int);void timer_stop(int);float timer_read(int);
void c_print_results(char*,char,int,int,int,int,int,float,float,char*,int,char*,char*,char*,char*,char*,char*,char*,char*,char*);
...
static void mg3P(float ****u, float ***v, float ****r, float a[4],
		 float c[4], int n1, int n2, int n3, int k);
static void psinv( float ***r, float ***u, int n1, int n2, int n3,
		   float c[4], int k);
static void resid( float ***u, float ***v, float ***r,
		   int n1, int n2, int n3, float a[4], int k );
static void rprj3( float ***r, int m1k, int m2k, int m3k,
		   float ***s, int m1j, int m2j, int m3j, int k );
static void interp( float ***z, int mm1, int mm2, int mm3,
		    float ***u, int n1, int n2, int n3, int k );
static void norm2u3(float ***r, int n1, int n2, int n3,
		    float *rnm2, float *rnmu, int nx, int ny, int nz);
static void rep_nrm(float ***u, int n1, int n2, int n3,
		    char *title, int kk);
static void comm3(float ***u, int n1, int n2, int n3, int kk);
static void zran3(double ***z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(float ***z, int n1, int n2, int n3);
static double power( double a, int n );
static void bubble( double ten[M][2], int j1[M][2], int j2[M][2],
		    int j3[M][2], int m, int ind );
static void zero3(float ***z, int n1, int n2, int n3);
static void nonzero(float ***z, int n1, int n2, int n3);
...
c-------------------------------------------------------------------*/
    ...
    u = (float ****)malloc((lt+1)*sizeof(float ***));
    for (l = lt; l >=1; l--) {
	u[l] = (float ***)malloc(m3[l]*sizeof(float **));
	for (k = 0; k < m3[l]; k++) {
	    u[l][k] = (float **)malloc(m2[l]*sizeof(float *));
	    for (j = 0; j < m2[l]; j++) {
		u[l][k][j] = (float *)malloc(m1[l]*sizeof(float));
	    }
	}
    }
    v = (float ***)malloc(m3[lt]*sizeof(float **));
    for (k = 0; k < m3[lt]; k++) {
	v[k] = (float **)malloc(m2[lt]*sizeof(float *));
	for (j = 0; j < m2[lt]; j++) {
	    v[k][j] = (float *)malloc(m1[lt]*sizeof(float));
	}
    }
    ...
    r = (float ****)malloc((lt+1)*sizeof(float ***));
    for (l = lt; l >=1; l--) {
	r[l] = (float ***)malloc(m3[l]*sizeof(float **));
	for (k = 0; k < m3[l]; k++) {
	    r[l][k] = (float **)malloc(m2[l]*sizeof(float *));
	    for (j = 0; j < m2[l]; j++) {
		r[l][k][j] = (float *)malloc(m1[l]*sizeof(float));
	    }
	}
    }
    ...
    { int _a,_b,_c; for(_a=0;_a<m3[lt];_a++)for(_b=0;_b<m2[lt];_b++)for(_c=0;_c<m1[lt];_c++) v[_a][_b][_c]=(float)vd[_a][_b][_c]; }
...
#pragma omp master  
  nthreads = omp_get_num_threads();
    ...
    if (Class != 'U') {
	if (Class == 'S') {
            verify_value = 0.530770700573e-04f;
	} else if (Class == 'W') {
            verify_value = 0.250391406439e-17f;  /* 40 iterations*/
/*				0.183103168997d-044 iterations*/
	} else if (Class == 'A') {
            verify_value = 0.2433365309e-5f;
        } else if (Class == 'B') {
            verify_value = 0.180056440132e-5f;
        } else if (Class == 'C') {
            verify_value = 0.570674826298e-06f;
	}

	if ( fabsf( rnm2 - verify_value ) <= epsilon ) {
            verified = TRUE;
...
/*C             i1 = 2*j1-1*/
		...
		s[j3][j2][j1] =
		    0.5f * r[i3+1][i2+1][i1+1]
		    + 0.25f * ( r[i3+1][i2+1][i1] + r[i3+1][i2+1][i1+2] + x2)
		    + 0.125f * ( x1[i1] + x1[i1+2] + y2)
		    + 0.0625f * ( y1[i1] + y1[i1+2] );
	    }
	}
    }
...
#pragma omp for
	for ( i3 = d3; i3 <= mm3-1; i3++) {
		...
		for ( i1 = 1; i1 <= mm1-1; i1++) {
			...
			u[2*i3-d3-1][2*i2-d2-1][2*i1-t1-1]
			+0.5f*(z[i3-1][i2-1][i1]+z[i3-1][i2-1][i1-1]);
		}
	    }
		...
		for ( i1 = d1; i1 <= mm1-1; i1++) {
			...
			u[2*i3-d3-1][2*i2-t2-1][2*i1-d1-1]
			+0.5f*(z[i3-1][i2][i1-1]+z[i3-1][i2-1][i1-1]);
		}
		for ( i1 = 1; i1 <= mm1-1; i1++) {
		    u[2*i3-d3-1][2*i2-t2-1][2*i1-t1-1] =
			u[2*i3-d3-1][2*i2-t2-1][2*i1-t1-1]
			+0.25f*(z[i3-1][i2][i1]+z[i3-1][i2-1][i1]
...
#pragma omp for nowait
	for ( i3 = 1; i3 <= mm3-1; i3++) {
		...
		for ( i1 = d1; i1 <= mm1-1; i1++) {
			...
			u[2*i3-t3-1][2*i2-d2-1][2*i1-d1-1]
			+0.5f*(z[i3][i2-1][i1-1]+z[i3-1][i2-1][i1-1]);
		}
		for ( i1 = 1; i1 <= mm1-1; i1++) {
		    u[2*i3-t3-1][2*i2-d2-1][2*i1-t1-1] =
			u[2*i3-t3-1][2*i2-d2-1][2*i1-t1-1]
			+0.25f*(z[i3][i2-1][i1]+z[i3][i2-1][i1-1]
		...
		for ( i1 = d1; i1 <= mm1-1; i1++) {
			...
			u[2*i3-t3-1][2*i2-t2-1][2*i1-d1-1]
			+0.25f*(z[i3][i2][i1-1]+z[i3][i2-1][i1-1]
			       +z[i3-1][i2][i1-1]+z[i3-1][i2-1][i1-1]);
		}
		for ( i1 = 1; i1 <= mm1-1; i1++) {
		    u[2*i3-t3-1][2*i2-t2-1][2*i1-t1-1] =
			u[2*i3-t3-1][2*i2-t2-1][2*i1-t1-1]
			+0.125f*(z[i3][i2][i1]+z[i3][i2-1][i1]
...
#include <sys/time.h>
static float _wt(void){struct timeval tv;static long s=-1;gettimeofday(&tv,(void*)0);if(s<0)s=tv.tv_sec;return (float)(tv.tv_sec-s)+(float)1.0e-6f*(float)tv.tv_usec;}
static float _st[64],_el[64];
void timer_clear(int n){_el[n]=(float)0;}
void timer_start(int n){_st[n]=_wt();}
void timer_stop(int n){float x=_wt();_el[n]+=x-_st[n];}
float timer_read(int n){return _el[n];}
void c_print_results(char*nm,char cl,int n1,int n2,int n3,int ni,int nt,float t,float mo,char*ot,int ps,char*ve,char*ct,char*cc,char*cl2,char*cli,char*ci,char*cf,char*clf,char*rd){
