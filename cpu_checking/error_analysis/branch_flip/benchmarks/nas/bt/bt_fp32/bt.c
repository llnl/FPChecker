/*--------------------------------------------------------------------
  NPB BT (NPB3.0-omp-C) -- standalone single-file FP64 build.
  All physics variables, constants, and math functions are double.
  CLASS = S (grid 12^3, NITER=60, DT=0.010). Serial (no OpenMP).
  Build:  cc -O3 -std=c99 bt.c -lm -o bt_fp64
  NOTE: verify requires NITER=60 (reference table is locked to 60 steps).
--------------------------------------------------------------------*/
#include <stdio.h>
#include "NAS_Precision.h"
#include <stdlib.h>
#include <math.h>
...
#define TRUE 1
#define FALSE 0
#define PRECISION_NAME "fp64"
typedef nas_real TIMET;   /* timer/report type (own precision) */

#define max(a,b) (((a)>(b))?(a):(b))
#define min(a,b) (((a)<(b))?(a):(b))
#define pow2(a) ((a)*(a))

void timer_clear(int);void timer_start(int);void timer_stop(int);TIMET timer_read(int);
void c_print_results(char*,char,int,int,int,int,int,TIMET,TIMET,char*,int,char*,char*,char*,char*,char*,char*,char*,char*,char*);

/* ---- npbparams.h : NPB BT, CLASS = S (generated) ---- */
...
static nas_real tx1, tx2, tx3, ty1, ty2, ty3, tz1, tz2, tz3;
static nas_real dx1, dx2, dx3, dx4, dx5;
static nas_real dy1, dy2, dy3, dy4, dy5;
static nas_real dz1, dz2, dz3, dz4, dz5;
static nas_real dssp, dt;
static nas_real ce[5][13];	/* ce(5,13) */
static nas_real dxmax, dymax, dzmax;
static nas_real xxcon1, xxcon2, xxcon3, xxcon4, xxcon5;
static nas_real dx1tx1, dx2tx1, dx3tx1, dx4tx1, dx5tx1;
static nas_real yycon1, yycon2, yycon3, yycon4, yycon5;
static nas_real dy1ty1, dy2ty1, dy3ty1, dy4ty1, dy5ty1;
static nas_real zzcon1, zzcon2, zzcon3, zzcon4, zzcon5;
static nas_real dz1tz1, dz2tz1, dz3tz1, dz4tz1, dz5tz1;
static nas_real dnxm1, dnym1, dnzm1, c1c2, c1c5, c3c4, c1345;
static nas_real conz1, c1, c2, c3, c4, c5, c4dssp, c5dssp, dtdssp;
static nas_real dttx1, dttx2, dtty1, dtty2, dttz1, dttz2;
static nas_real c2dttx1, c2dtty1, c2dttz1, comz1, comz4, comz5, comz6;
static nas_real c3c4tx3, c3c4ty3, c3c4tz3, c2iv, con43, con16;
...
/* COMMON block: fields */
static nas_real us[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real vs[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real ws[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real qs[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real rho_i[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real square[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1];
static nas_real forcing[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1][5+1];
static nas_real u[(IMAX+1)/2*2+1][(JMAX+1)/2*2+1][(KMAX+1)/2*2+1][5];
static nas_real rhs[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1][5];
static nas_real lhs[IMAX/2*2+1][JMAX/2*2+1][KMAX/2*2+1][3][5][5];

/* COMMON block: work_1d */
static nas_real cuf[PROBLEM_SIZE];
static nas_real q[PROBLEM_SIZE];
static nas_real ue[PROBLEM_SIZE][5];
static nas_real buf[PROBLEM_SIZE][5];
...
static nas_real fjac[IMAX/2*2+1][JMAX/2*2+1][KMAX-1+1][5][5];
/* fjac(5, 5, 0:IMAX/2*2, 0:JMAX/2*2, 0:KMAX-1) */
static nas_real njac[IMAX/2*2+1][JMAX/2*2+1][KMAX-1+1][5][5];
/* njac(5, 5, 0:IMAX/2*2, 0:JMAX/2*2, 0:KMAX-1) */
static nas_real tmp1, tmp2, tmp3;
...
static void error_norm(nas_real rms[5]);
static void rhs_norm(nas_real rms[5]);
static void exact_rhs(void);
static void exact_solution(nas_real xi, nas_real eta, nas_real zeta,
			   nas_real dtemp[5]);
...
static void matvec_sub(nas_real ablock[5][5], nas_real avec[5], nas_real bvec[5]);
static void matmul_sub(nas_real ablock[5][5], nas_real bblock[5][5],
		       nas_real cblock[5][5]);
static void binvcrhs(nas_real lhs[5][5], nas_real c[5][5], nas_real r[5]);
static void binvrhs(nas_real lhs[5][5], nas_real r[5]);
...
c-------------------------------------------------------------------*/
  ...
  if (fp != NULL) {
    ...
    fscanf(fp, "%d", &niter);
    while (fgetc(fp) != '\n');
    { double t_; fscanf(fp, "%lg", &t_); dt = (nas_real)t_; }
    while (fgetc(fp) != '\n');
    fscanf(fp, "%d%d%d",
...
#pragma omp master  
  ...
  navg = (grid_points[0]+grid_points[1]+grid_points[2])/NAS_R(3.0);
  if ( tmax != NAS_R(0.0) ) {
    mflops = NAS_R(1.0e-6)*(nas_real)niter*
	(NAS_R(3478.8)*(nas_real)n3-NAS_R(17655.7)*pow2(navg)+NAS_R(28023.7)*navg) / tmax;
  } else {
    mflops = NAS_R(0.0);
  }
...
c-------------------------------------------------------------------*/
  ...
  int i, j, k, m, d;
  nas_real xi, eta, zeta, u_exact[5], add;
  ...
  for (i = 0; i < grid_points[0]; i++) {
    ...
    for (j = 0; j < grid_points[1]; j++) {
      eta = (nas_real)j * dnym1;
      for (k = 0; k < grid_points[2]; k++) {
	zeta = (nas_real)k * dnzm1;
  ...
  for (m = 0; m < 5; m++) {
    for (d = 0; d <= 2; d++) {
      rms[m] = rms[m] / (nas_real)(grid_points[d]-2);
    }
    rms[m] = nas_sqrt(rms[m]);
  }
}
...
c-------------------------------------------------------------------*/
  ...
  nas_real dtemp[5], xi, eta, zeta, dtpp;
...
#pragma omp for
  for (j = 1; j < grid_points[1]-1; j++) {
	...
	buf[i][0] = cuf[i] + buf[i][2] * buf[i][2] + 
	  buf[i][3] * buf[i][3];
	q[i] = NAS_R(0.5)*(buf[i][1]*ue[i][1] + buf[i][2]*ue[i][2] +
		    buf[i][3]*ue[i][3]);
      }
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  ...
	  dx1tx1*(ue[ip1][0]-NAS_R(2.0)*ue[i][0]+ue[im1][0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tx2 * ((ue[ip1][1]*buf[ip1][1]+c2*(ue[ip1][4]-q[ip1]))-
		 (ue[im1][1]*buf[im1][1]+c2*(ue[im1][4]-q[im1])))+
	  xxcon1*(buf[ip1][1]-NAS_R(2.0)*buf[i][1]+buf[im1][1])+
	  dx2tx1*( ue[ip1][1]-NAS_R(2.0)* ue[i][1]+ ue[im1][1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tx2 * (ue[ip1][2]*buf[ip1][1]-ue[im1][2]*buf[im1][1])+
	  xxcon2*(buf[ip1][2]-NAS_R(2.0)*buf[i][2]+buf[im1][2])+
	  dx3tx1*( ue[ip1][2]-NAS_R(2.0)* ue[i][2]+ ue[im1][2]);
                  
	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tx2*(ue[ip1][3]*buf[ip1][1]-ue[im1][3]*buf[im1][1])+
	  xxcon2*(buf[ip1][3]-NAS_R(2.0)*buf[i][3]+buf[im1][3])+
	  dx4tx1*( ue[ip1][3]-NAS_R(2.0)* ue[i][3]+ ue[im1][3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tx2*(buf[ip1][1]*(c1*ue[ip1][4]-c2*q[ip1])-
	       buf[im1][1]*(c1*ue[im1][4]-c2*q[im1]))+
	  NAS_R(0.5)*xxcon3*(buf[ip1][0]-NAS_R(2.0)*buf[i][0]+buf[im1][0])+
	  xxcon4*(cuf[ip1]-NAS_R(2.0)*cuf[i]+cuf[im1])+
	  xxcon5*(buf[ip1][4]-NAS_R(2.0)*buf[i][4]+buf[im1][4])+
	  dx5tx1*( ue[ip1][4]-NAS_R(2.0)* ue[i][4]+ ue[im1][4]);
      }
...
c-------------------------------------------------------------------*/
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (NAS_R(5.0)*ue[i][m] - NAS_R(4.0)*ue[i+1][m] +ue[i+2][m]);
	i = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-NAS_R(4.0)*ue[i-1][m] + NAS_R(6.0)*ue[i][m] -
	    NAS_R(4.0)*ue[i+1][m] +     ue[i+2][m]);
      }

      for (m = 0; m < 5; m++) {
	for (i = 1*3; i <= grid_points[0]-3*1-1; i++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[i-2][m] - NAS_R(4.0)*ue[i-1][m] +
	     NAS_R(6.0)*ue[i][m] - NAS_R(4.0)*ue[i+1][m] + ue[i+2][m]);
	}
      }
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[i-2][m] - NAS_R(4.0)*ue[i-1][m] +
	   NAS_R(6.0)*ue[i][m] - NAS_R(4.0)*ue[i+1][m]);
	i = grid_points[0]-2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[i-2][m] - NAS_R(4.0)*ue[i-1][m] + NAS_R(5.0)*ue[i][m]);
      }
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	buf[j][0] = cuf[j] + buf[j][1] * buf[j][1] + 
	  buf[j][3] * buf[j][3];
	q[j] = NAS_R(0.5)*(buf[j][1]*ue[j][1] + buf[j][2]*ue[j][2] +
		    buf[j][3]*ue[j][3]);
      }
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  ...
	  dy1ty1*(ue[jp1][0]-NAS_R(2.0)*ue[j][0]+ue[jm1][0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  ty2*(ue[jp1][1]*buf[jp1][2]-ue[jm1][1]*buf[jm1][2])+
	  yycon2*(buf[jp1][1]-NAS_R(2.0)*buf[j][1]+buf[jm1][1])+
	  dy2ty1*( ue[jp1][1]-NAS_R(2.0)* ue[j][1]+ ue[jm1][1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  ty2*((ue[jp1][2]*buf[jp1][2]+c2*(ue[jp1][4]-q[jp1]))-
	       (ue[jm1][2]*buf[jm1][2]+c2*(ue[jm1][4]-q[jm1])))+
	  yycon1*(buf[jp1][2]-NAS_R(2.0)*buf[j][2]+buf[jm1][2])+
	  dy3ty1*( ue[jp1][2]-NAS_R(2.0)*ue[j][2] +ue[jm1][2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  ty2*(ue[jp1][3]*buf[jp1][2]-ue[jm1][3]*buf[jm1][2])+
	  yycon2*(buf[jp1][3]-NAS_R(2.0)*buf[j][3]+buf[jm1][3])+
	  dy4ty1*( ue[jp1][3]-NAS_R(2.0)*ue[j][3]+ ue[jm1][3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  ty2*(buf[jp1][2]*(c1*ue[jp1][4]-c2*q[jp1])-
	       buf[jm1][2]*(c1*ue[jm1][4]-c2*q[jm1]))+
	  NAS_R(0.5)*yycon3*(buf[jp1][0]-NAS_R(2.0)*buf[j][0]+
                      buf[jm1][0])+
	  yycon4*(cuf[jp1]-NAS_R(2.0)*cuf[j]+cuf[jm1])+
	  yycon5*(buf[jp1][4]-NAS_R(2.0)*buf[j][4]+buf[jm1][4])+
	  dy5ty1*(ue[jp1][4]-NAS_R(2.0)*ue[j][4]+ue[jm1][4]);
      }
...
c-------------------------------------------------------------------*/
	...
	j = 1;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (NAS_R(5.0)*ue[j][m] - NAS_R(4.0)*ue[j+1][m] +ue[j+2][m]);
	j = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-NAS_R(4.0)*ue[j-1][m] + NAS_R(6.0)*ue[j][m] -
	   NAS_R(4.0)*ue[j+1][m] +       ue[j+2][m]);
      }

      for (m = 0; m < 5; m++) {
	for (j = 1*3; j <= grid_points[1]-3*1-1; j++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[j-2][m] - NAS_R(4.0)*ue[j-1][m] +
	     NAS_R(6.0)*ue[j][m] - NAS_R(4.0)*ue[j+1][m] + ue[j+2][m]);
	}
      }
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[j-2][m] - NAS_R(4.0)*ue[j-1][m] +
	   NAS_R(6.0)*ue[j][m] - NAS_R(4.0)*ue[j+1][m]);
	j = grid_points[1]-2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[j-2][m] - NAS_R(4.0)*ue[j-1][m] + NAS_R(5.0)*ue[j][m]);
      }
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	buf[k][0] = cuf[k] + buf[k][1] * buf[k][1] + 
	  buf[k][2] * buf[k][2];
	q[k] = NAS_R(0.5)*(buf[k][1]*ue[k][1] + buf[k][2]*ue[k][2] +
		    buf[k][3]*ue[k][3]);
      }
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  ...
	  dz1tz1*(ue[kp1][0]-NAS_R(2.0)*ue[k][0]+ue[km1][0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tz2 * (ue[kp1][1]*buf[kp1][3]-ue[km1][1]*buf[km1][3])+
	  zzcon2*(buf[kp1][1]-NAS_R(2.0)*buf[k][1]+buf[km1][1])+
	  dz2tz1*( ue[kp1][1]-NAS_R(2.0)* ue[k][1]+ ue[km1][1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tz2 * (ue[kp1][2]*buf[kp1][3]-ue[km1][2]*buf[km1][3])+
	  zzcon2*(buf[kp1][2]-NAS_R(2.0)*buf[k][2]+buf[km1][2])+
	  dz3tz1*(ue[kp1][2]-NAS_R(2.0)*ue[k][2]+ue[km1][2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tz2 * ((ue[kp1][3]*buf[kp1][3]+c2*(ue[kp1][4]-q[kp1]))-
		 (ue[km1][3]*buf[km1][3]+c2*(ue[km1][4]-q[km1])))+
	  zzcon1*(buf[kp1][3]-NAS_R(2.0)*buf[k][3]+buf[km1][3])+
	  dz4tz1*( ue[kp1][3]-NAS_R(2.0)*ue[k][3] +ue[km1][3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tz2 * (buf[kp1][3]*(c1*ue[kp1][4]-c2*q[kp1])-
		 buf[km1][3]*(c1*ue[km1][4]-c2*q[km1]))+
	  NAS_R(0.5)*zzcon3*(buf[kp1][0]-NAS_R(2.0)*buf[k][0]
                      +buf[km1][0])+
	  zzcon4*(cuf[kp1]-NAS_R(2.0)*cuf[k]+cuf[km1])+
	  zzcon5*(buf[kp1][4]-NAS_R(2.0)*buf[k][4]+buf[km1][4])+
	  dz5tz1*( ue[kp1][4]-NAS_R(2.0)*ue[k][4]+ ue[km1][4]);
      }
...
c-------------------------------------------------------------------*/
	...
	k = 1;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (NAS_R(5.0)*ue[k][m] - NAS_R(4.0)*ue[k+1][m] +ue[k+2][m]);
	k = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-NAS_R(4.0)*ue[k-1][m] + NAS_R(6.0)*ue[k][m] -
	   NAS_R(4.0)*ue[k+1][m] +       ue[k+2][m]);
      }

      for (m = 0; m < 5; m++) {
	for (k = 1*3; k <= grid_points[2]-3*1-1; k++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[k-2][m] - NAS_R(4.0)*ue[k-1][m] +
	     NAS_R(6.0)*ue[k][m] - NAS_R(4.0)*ue[k+1][m] + ue[k+2][m]);
	}
      }
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[k-2][m] - NAS_R(4.0)*ue[k-1][m] +
	   NAS_R(6.0)*ue[k][m] - NAS_R(4.0)*ue[k+1][m]);
	k = grid_points[2]-2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[k-2][m] - NAS_R(4.0)*ue[k-1][m] + NAS_R(5.0)*ue[k][m]);
      }
...
#pragma omp for  
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	for (m = 0; m < 5; m++) {
	  forcing[i][j][k][m] = -NAS_R(1.0) * forcing[i][j][k][m];
	}
      }
    }
  }
...
c-------------------------------------------------------------------*/
  ...
  nas_real xi, eta, zeta, Pface[2][3][5], Pxi, Peta, Pzeta, temp[5];
...
#pragma omp for
  for (i = 0; i < grid_points[0]; i++) {
	...
	for (ix = 0; ix < 2; ix++) {
	  exact_solution((nas_real)ix, eta, zeta, 
                         &(Pface[ix][0][0]));
	}

	for (iy = 0; iy < 2; iy++) {
	  exact_solution(xi, (nas_real)iy , zeta, 
                         &Pface[iy][1][0]);
	}

	for (iz = 0; iz < 2; iz++) {
	  exact_solution(xi, eta, (nas_real)iz,   
                         &Pface[iz][2][0]);
	}

	for (m = 0; m < 5; m++) {
	  Pxi   = xi   * Pface[1][0][m] + 
	    (NAS_R(1.0)-xi)   * Pface[0][0][m];
	  Peta  = eta  * Pface[1][1][m] + 
	    (NAS_R(1.0)-eta)  * Pface[0][1][m];
	  Pzeta = zeta * Pface[1][2][m] + 
	    (NAS_R(1.0)-zeta) * Pface[0][2][m];
...
#pragma omp for
  for (i = 0; i < grid_points[0]; i++) {
	...
	for (m = 0; m < 5; m++) {
	  for (n = 0; n < 5; n++) {
	    lhs[i][j][k][0][m][n] = NAS_R(0.0);
	    lhs[i][j][k][1][m][n] = NAS_R(0.0);
	    lhs[i][j][k][2][m][n] = NAS_R(0.0);
	  }
	}
      }
    }
...
#pragma omp for  
  for (j = 1; j < grid_points[1]-1; j++) {
	...
	tmp1 = NAS_R(1.0) / u[i][j][k][0];
	tmp2 = tmp1 * tmp1;
	tmp3 = tmp1 * tmp2;
/*--------------------------------------------------------------------
c     
c-------------------------------------------------------------------*/
	fjac[ i][ j][ k][0][0] = NAS_R(0.0);
	fjac[ i][ j][ k][0][1] = NAS_R(1.0);
	fjac[ i][ j][ k][0][2] = NAS_R(0.0);
	fjac[ i][ j][ k][0][3] = NAS_R(0.0);
	fjac[ i][ j][ k][0][4] = NAS_R(0.0);

	fjac[ i][ j][ k][1][0] = -(u[i][j][k][1] * tmp2 * 
				    u[i][j][k][1])
	  + c2 * NAS_R(0.50) * (u[i][j][k][1] * u[i][j][k][1]
		       + u[i][j][k][2] * u[i][j][k][2]
		       + u[i][j][k][3] * u[i][j][k][3] ) * tmp2;
	fjac[i][j][k][1][1] = ( NAS_R(2.0) - c2 )
	  * ( u[i][j][k][1] / u[i][j][k][0] );
	...
	njac[i][j][k][0][0] = NAS_R(0.0);
	njac[i][j][k][0][1] = NAS_R(0.0);
	njac[i][j][k][0][2] = NAS_R(0.0);
	njac[i][j][k][0][3] = NAS_R(0.0);
	njac[i][j][k][0][4] = NAS_R(0.0);

	njac[i][j][k][1][0] = - con43 * c3c4 * tmp2 * u[i][j][k][1];
	njac[i][j][k][1][1] =   con43 * c3c4 * tmp1;
	njac[i][j][k][1][2] =   NAS_R(0.0);
	njac[i][j][k][1][3] =   NAS_R(0.0);
	njac[i][j][k][1][4] =   NAS_R(0.0);

	njac[i][j][k][2][0] = - c3c4 * tmp2 * u[i][j][k][2];
	njac[i][j][k][2][1] =   NAS_R(0.0);
	njac[i][j][k][2][2] =   c3c4 * tmp1;
	njac[i][j][k][2][3] =   NAS_R(0.0);
	njac[i][j][k][2][4] =   NAS_R(0.0);

	njac[i][j][k][3][0] = - c3c4 * tmp2 * u[i][j][k][3];
	njac[i][j][k][3][1] =   NAS_R(0.0);
	njac[i][j][k][3][2] =   NAS_R(0.0);
	njac[i][j][k][3][3] =   c3c4 * tmp1;
	njac[i][j][k][3][4] =   NAS_R(0.0);
...
c-------------------------------------------------------------------*/
	...
	lhs[i][j][k][BB][0][0] = NAS_R(1.0)
	  + tmp1 * NAS_R(2.0) * njac[i][j][k][0][0]
	  + tmp1 * NAS_R(2.0) * dx1;
	lhs[i][j][k][BB][0][1] = tmp1 * NAS_R(2.0) * njac[i][j][k][0][1];
	lhs[i][j][k][BB][0][2] = tmp1 * NAS_R(2.0) * njac[i][j][k][0][2];
	lhs[i][j][k][BB][0][3] = tmp1 * NAS_R(2.0) * njac[i][j][k][0][3];
	lhs[i][j][k][BB][0][4] = tmp1 * NAS_R(2.0) * njac[i][j][k][0][4];

	lhs[i][j][k][BB][1][0] = tmp1 * NAS_R(2.0) * njac[i][j][k][1][0];
	lhs[i][j][k][BB][1][1] = NAS_R(1.0)
	  + tmp1 * NAS_R(2.0) * njac[i][j][k][1][1]
	  + tmp1 * NAS_R(2.0) * dx2;
	lhs[i][j][k][BB][1][2] = tmp1 * NAS_R(2.0) * njac[i][j][k][1][2];
	lhs[i][j][k][BB][1][3] = tmp1 * NAS_R(2.0) * njac[i][j][k][1][3];
	lhs[i][j][k][BB][1][4] = tmp1 * NAS_R(2.0) * njac[i][j][k][1][4];

	lhs[i][j][k][BB][2][0] = tmp1 * NAS_R(2.0) * njac[i][j][k][2][0];
	lhs[i][j][k][BB][2][1] = tmp1 * NAS_R(2.0) * njac[i][j][k][2][1];
	lhs[i][j][k][BB][2][2] = NAS_R(1.0)
	  + tmp1 * NAS_R(2.0) * njac[i][j][k][2][2]
	  + tmp1 * NAS_R(2.0) * dx3;
	lhs[i][j][k][BB][2][3] = tmp1 * NAS_R(2.0) * njac[i][j][k][2][3];
	lhs[i][j][k][BB][2][4] = tmp1 * NAS_R(2.0) * njac[i][j][k][2][4];

	lhs[i][j][k][BB][3][0] = tmp1 * NAS_R(2.0) * njac[i][j][k][3][0];
	lhs[i][j][k][BB][3][1] = tmp1 * NAS_R(2.0) * njac[i][j][k][3][1];
	lhs[i][j][k][BB][3][2] = tmp1 * NAS_R(2.0) * njac[i][j][k][3][2];
	lhs[i][j][k][BB][3][3] = NAS_R(1.0)
	  + tmp1 * NAS_R(2.0) * njac[i][j][k][3][3]
	  + tmp1 * NAS_R(2.0) * dx4;
	lhs[i][j][k][BB][3][4] = tmp1 * NAS_R(2.0) * njac[i][j][k][3][4];

	lhs[i][j][k][BB][4][0] = tmp1 * NAS_R(2.0) * njac[i][j][k][4][0];
	lhs[i][j][k][BB][4][1] = tmp1 * NAS_R(2.0) * njac[i][j][k][4][1];
	lhs[i][j][k][BB][4][2] = tmp1 * NAS_R(2.0) * njac[i][j][k][4][2];
	lhs[i][j][k][BB][4][3] = tmp1 * NAS_R(2.0) * njac[i][j][k][4][3];
	lhs[i][j][k][BB][4][4] = NAS_R(1.0)
	  + tmp1 * NAS_R(2.0) * njac[i][j][k][4][4]
	  + tmp1 * NAS_R(2.0) * dx5;
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	fjac[ i][ j][ k][0][0] = NAS_R(0.0);
	fjac[ i][ j][ k][0][1] = NAS_R(0.0);
	fjac[ i][ j][ k][0][2] = NAS_R(1.0);
	fjac[ i][ j][ k][0][3] = NAS_R(0.0);
	fjac[ i][ j][ k][0][4] = NAS_R(0.0);
	...
	fjac[i][j][k][1][3] = NAS_R(0.0);
	fjac[i][j][k][1][4] = NAS_R(0.0);

	fjac[i][j][k][2][0] = - ( u[i][j][k][2]*u[i][j][k][2]*tmp2)
	  + NAS_R(0.50) * c2 * ( (  u[i][j][k][1] * u[i][j][k][1]
			     + u[i][j][k][2] * u[i][j][k][2]
			     + u[i][j][k][3] * u[i][j][k][3] )
			  * tmp2 );
	fjac[i][j][k][2][1] = - c2 *  u[i][j][k][1] * tmp1;
	fjac[i][j][k][2][2] = ( NAS_R(2.0) - c2 )
	  *  u[i][j][k][2] * tmp1;
...
static void compute_rhs(void) {
  ...
  int i, j, k, m;
  nas_real rho_inv, uijk, up1, um1, vijk, vp1, vm1, wijk, wp1, wm1;
...
#pragma omp for nowait
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
	rho_inv = NAS_R(1.0)/u[i][j][k][0];
	rho_i[i][j][k] = rho_inv;
	us[i][j][k] = u[i][j][k][1] * rho_inv;
	vs[i][j][k] = u[i][j][k][2] * rho_inv;
	ws[i][j][k] = u[i][j][k][3] * rho_inv;
	square[i][j][k] = NAS_R(0.5) * (u[i][j][k][1]*u[i][j][k][1] + 
				 u[i][j][k][2]*u[i][j][k][2] +
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	rhs[i][j][k][0] = rhs[i][j][k][0] + dx1tx1 * 
	  (u[i+1][j][k][0] - NAS_R(2.0)*u[i][j][k][0] + 
	   u[i-1][j][k][0]) -
	  tx2 * (u[i+1][j][k][1] - u[i-1][j][k][1]);

	rhs[i][j][k][1] = rhs[i][j][k][1] + dx2tx1 * 
	  (u[i+1][j][k][1] - NAS_R(2.0)*u[i][j][k][1] + 
	   u[i-1][j][k][1]) +
	  xxcon2*con43 * (up1 - NAS_R(2.0)*uijk + um1) -
	  tx2 * (u[i+1][j][k][1]*up1 - 
		 u[i-1][j][k][1]*um1 +
	...
	rhs[i][j][k][2] = rhs[i][j][k][2] + dx3tx1 * 
	  (u[i+1][j][k][2] - NAS_R(2.0)*u[i][j][k][2] +
	   u[i-1][j][k][2]) +
	  xxcon2 * (vs[i+1][j][k] - NAS_R(2.0)*vs[i][j][k] +
		    vs[i-1][j][k]) -
	  tx2 * (u[i+1][j][k][2]*up1 - 
		 u[i-1][j][k][2]*um1);

	rhs[i][j][k][3] = rhs[i][j][k][3] + dx4tx1 * 
	  (u[i+1][j][k][3] - NAS_R(2.0)*u[i][j][k][3] +
	   u[i-1][j][k][3]) +
	  xxcon2 * (ws[i+1][j][k] - NAS_R(2.0)*ws[i][j][k] +
		    ws[i-1][j][k]) -
	  tx2 * (u[i+1][j][k][3]*up1 - 
		 u[i-1][j][k][3]*um1);

	rhs[i][j][k][4] = rhs[i][j][k][4] + dx5tx1 * 
	  (u[i+1][j][k][4] - NAS_R(2.0)*u[i][j][k][4] +
	   u[i-1][j][k][4]) +
	  xxcon3 * (qs[i+1][j][k] - NAS_R(2.0)*qs[i][j][k] +
		    qs[i-1][j][k]) +
	  xxcon4 * (up1*up1 -       NAS_R(2.0)*uijk*uijk + 
		    um1*um1) +
	  xxcon5 * (u[i+1][j][k][4]*rho_i[i+1][j][k] - 
		    NAS_R(2.0)*u[i][j][k][4]*rho_i[i][j][k] +
...
#pragma omp for nowait
  for (j = 1; j < grid_points[1]-1; j++) {
	...
	rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
	  ( NAS_R(5.0)*u[i][j][k][m] - NAS_R(4.0)*u[i+1][j][k][m] +
	    u[i+2][j][k][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	  (-NAS_R(4.0)*u[i-1][j][k][m] + NAS_R(6.0)*u[i][j][k][m] -
	   NAS_R(4.0)*u[i+1][j][k][m] + u[i+2][j][k][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (i = 3; i < grid_points[0]-3; i++) {
	...
	for (m = 0; m < 5; m++) {
	  rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	    (  u[i-2][j][k][m] - NAS_R(4.0)*u[i-1][j][k][m] + 
	       NAS_R(6.0)*u[i][j][k][m] - NAS_R(4.0)*u[i+1][j][k][m] + 
	       u[i+2][j][k][m] );
	}
      }
    }
...
#pragma omp for
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
	  ( u[i-2][j][k][m] - NAS_R(4.)*u[i-1][j][k][m] +
	    NAS_R(5.0)*u[i][j][k][m] );
      }
    }
  }
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	rhs[i][j][k][0] = rhs[i][j][k][0] + dy1ty1 * 
	  (u[i][j+1][k][0] - NAS_R(2.0)*u[i][j][k][0] + 
	   u[i][j-1][k][0]) -
	  ty2 * (u[i][j+1][k][2] - u[i][j-1][k][2]);
	rhs[i][j][k][1] = rhs[i][j][k][1] + dy2ty1 * 
	  (u[i][j+1][k][1] - NAS_R(2.0)*u[i][j][k][1] + 
	   u[i][j-1][k][1]) +
	  yycon2 * (us[i][j+1][k] - NAS_R(2.0)*us[i][j][k] + 
		    us[i][j-1][k]) -
	  ty2 * (u[i][j+1][k][1]*vp1 - 
		 u[i][j-1][k][1]*vm1);
	rhs[i][j][k][2] = rhs[i][j][k][2] + dy3ty1 * 
	  (u[i][j+1][k][2] - NAS_R(2.0)*u[i][j][k][2] + 
	   u[i][j-1][k][2]) +
	  yycon2*con43 * (vp1 - NAS_R(2.0)*vijk + vm1) -
	  ty2 * (u[i][j+1][k][2]*vp1 - 
		 u[i][j-1][k][2]*vm1 +
	...
	rhs[i][j][k][3] = rhs[i][j][k][3] + dy4ty1 * 
	  (u[i][j+1][k][3] - NAS_R(2.0)*u[i][j][k][3] + 
	   u[i][j-1][k][3]) +
	  yycon2 * (ws[i][j+1][k] - NAS_R(2.0)*ws[i][j][k] + 
		    ws[i][j-1][k]) -
	  ty2 * (u[i][j+1][k][3]*vp1 - 
		 u[i][j-1][k][3]*vm1);
	rhs[i][j][k][4] = rhs[i][j][k][4] + dy5ty1 * 
	  (u[i][j+1][k][4] - NAS_R(2.0)*u[i][j][k][4] + 
	   u[i][j-1][k][4]) +
	  yycon3 * (qs[i][j+1][k] - NAS_R(2.0)*qs[i][j][k] + 
		    qs[i][j-1][k]) +
	  yycon4 * (vp1*vp1       - NAS_R(2.0)*vijk*vijk + 
		    vm1*vm1) +
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
	  ( NAS_R(5.0)*u[i][j][k][m] - NAS_R(4.0)*u[i][j+1][k][m] +
	    u[i][j+2][k][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	  (-NAS_R(4.0)*u[i][j-1][k][m] + NAS_R(6.0)*u[i][j][k][m] -
	   NAS_R(4.0)*u[i][j+1][k][m] + u[i][j+2][k][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	for (m = 0; m < 5; m++) {
	  rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	    (  u[i][j-2][k][m] - NAS_R(4.0)*u[i][j-1][k][m] + 
	       NAS_R(6.0)*u[i][j][k][m] - NAS_R(4.0)*u[i][j+1][k][m] + 
	       u[i][j+2][k][m] );
	}
      }
    }
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
	  ( u[i][j-2][k][m] - NAS_R(4.)*u[i][j-1][k][m] +
	    NAS_R(5.)*u[i][j][k][m] );
      }
    }
  }
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	rhs[i][j][k][0] = rhs[i][j][k][0] + dz1tz1 * 
	  (u[i][j][k+1][0] - NAS_R(2.0)*u[i][j][k][0] + 
	   u[i][j][k-1][0]) -
	  tz2 * (u[i][j][k+1][3] - u[i][j][k-1][3]);
	rhs[i][j][k][1] = rhs[i][j][k][1] + dz2tz1 * 
	  (u[i][j][k+1][1] - NAS_R(2.0)*u[i][j][k][1] + 
	   u[i][j][k-1][1]) +
	  zzcon2 * (us[i][j][k+1] - NAS_R(2.0)*us[i][j][k] + 
		    us[i][j][k-1]) -
	  tz2 * (u[i][j][k+1][1]*wp1 - 
		 u[i][j][k-1][1]*wm1);
	rhs[i][j][k][2] = rhs[i][j][k][2] + dz3tz1 * 
	  (u[i][j][k+1][2] - NAS_R(2.0)*u[i][j][k][2] + 
	   u[i][j][k-1][2]) +
	  zzcon2 * (vs[i][j][k+1] - NAS_R(2.0)*vs[i][j][k] + 
		    vs[i][j][k-1]) -
	  tz2 * (u[i][j][k+1][2]*wp1 - 
		 u[i][j][k-1][2]*wm1);
	rhs[i][j][k][3] = rhs[i][j][k][3] + dz4tz1 * 
	  (u[i][j][k+1][3] - NAS_R(2.0)*u[i][j][k][3] + 
	   u[i][j][k-1][3]) +
	  zzcon2*con43 * (wp1 - NAS_R(2.0)*wijk + wm1) -
	  tz2 * (u[i][j][k+1][3]*wp1 - 
		 u[i][j][k-1][3]*wm1 +
	...
	rhs[i][j][k][4] = rhs[i][j][k][4] + dz5tz1 * 
	  (u[i][j][k+1][4] - NAS_R(2.0)*u[i][j][k][4] + 
	   u[i][j][k-1][4]) +
	  zzcon3 * (qs[i][j][k+1] - NAS_R(2.0)*qs[i][j][k] + 
		    qs[i][j][k-1]) +
	  zzcon4 * (wp1*wp1 - NAS_R(2.0)*wijk*wijk + 
		    wm1*wm1) +
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
	  ( NAS_R(5.0)*u[i][j][k][m] - NAS_R(4.0)*u[i][j][k+1][m] +
	    u[i][j][k+2][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	  (-NAS_R(4.0)*u[i][j][k-1][m] + NAS_R(6.0)*u[i][j][k][m] -
	   NAS_R(4.0)*u[i][j][k+1][m] + u[i][j][k+2][m]);
      }
    }
  }
...
#pragma omp for nowait
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	for (m = 0; m < 5; m++) {
	  rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
	    (  u[i][j][k-2][m] - NAS_R(4.0)*u[i][j][k-1][m] + 
	       NAS_R(6.0)*u[i][j][k][m] - NAS_R(4.0)*u[i][j][k+1][m] + 
	       u[i][j][k+2][m] );
	}
      }
    }
...
--------------------------------------------------------------------*/
  ...
  dnxm1 = NAS_R(1.0) / (nas_real)(grid_points[0]-1);
  dnym1 = NAS_R(1.0) / (nas_real)(grid_points[1]-1);
  dnzm1 = NAS_R(1.0) / (nas_real)(grid_points[2]-1);
  ...
  conz1 = (NAS_R(1.0)-c1c5);

  tx1 = NAS_R(1.0) / (dnxm1 * dnxm1);
  tx2 = NAS_R(1.0) / (NAS_R(2.0) * dnxm1);
  tx3 = NAS_R(1.0) / dnxm1;

  ty1 = NAS_R(1.0) / (dnym1 * dnym1);
  ty2 = NAS_R(1.0) / (NAS_R(2.0) * dnym1);
  ty3 = NAS_R(1.0) / dnym1;
      
  tz1 = NAS_R(1.0) / (dnzm1 * dnzm1);
  tz2 = NAS_R(1.0) / (NAS_R(2.0) * dnzm1);
  tz3 = NAS_R(1.0) / dnzm1;
  ...
  dssp = NAS_R(0.25) * max(dx1, max(dy1, dz1) );

  c4dssp = NAS_R(4.0) * dssp;
  c5dssp = NAS_R(5.0) * dssp;
  ...
  c2dttx1 = NAS_R(2.0)*dttx1;
  c2dtty1 = NAS_R(2.0)*dtty1;
  c2dttz1 = NAS_R(2.0)*dttz1;

  dtdssp = dt*dssp;

  comz1  = dtdssp;
  comz4  = NAS_R(4.0)*dtdssp;
  comz5  = NAS_R(5.0)*dtdssp;
  comz6  = NAS_R(6.0)*dtdssp;
  ...
  c2iv  = NAS_R(2.5);
  con43 = NAS_R(4.0)/NAS_R(3.0);
  con16 = NAS_R(1.0)/NAS_R(6.0);
...
c-------------------------------------------------------------------*/
  ...
  nas_real xcrref[5],xceref[5],xcrdif[5],xcedif[5], 
...
c-------------------------------------------------------------------*/
    xcrref[0] = NAS_R(1.7034283709541311e-01);
    xcrref[1] = NAS_R(1.2975252070034097e-02);
    xcrref[2] = NAS_R(3.2527926989486055e-02);
    xcrref[3] = NAS_R(2.6436421275166801e-02);
    xcrref[4] = NAS_R(1.9211784131744430e-01);

/*--------------------------------------------------------------------
c  Reference values of RMS-norms of solution error.
c-------------------------------------------------------------------*/
    xceref[0] = NAS_R(4.9976913345811579e-04);
    xceref[1] = NAS_R(4.5195666782961927e-05);
    xceref[2] = NAS_R(7.3973765172921357e-05);
    xceref[3] = NAS_R(7.3821238632439731e-05);
    xceref[4] = NAS_R(8.9269630987491446e-04);
...
c  Reference values of RMS-norms of residual.
c-------------------------------------------------------------------*/
      xcrref[0] = NAS_R(0.1125590409344e+03);
      xcrref[1] = NAS_R(0.1180007595731e+02);
      xcrref[2] = NAS_R(0.2710329767846e+02);
      xcrref[3] = NAS_R(0.2469174937669e+02);
      xcrref[4] = NAS_R(0.2638427874317e+03);

/*--------------------------------------------------------------------
c  Reference values of RMS-norms of solution error.
c-------------------------------------------------------------------*/
      xceref[0] = NAS_R(0.4419655736008e+01);
      xceref[1] = NAS_R(0.4638531260002e+00);
      xceref[2] = NAS_R(0.1011551749967e+01);
      xceref[3] = NAS_R(0.9235878729944e+00);
      xceref[4] = NAS_R(0.1018045837718e+02);
...
c  Reference values of RMS-norms of residual.
c-------------------------------------------------------------------*/
      xcrref[0] = NAS_R(1.0806346714637264e+02);
      xcrref[1] = NAS_R(1.1319730901220813e+01);
      xcrref[2] = NAS_R(2.5974354511582465e+01);
      xcrref[3] = NAS_R(2.3665622544678910e+01);
      xcrref[4] = NAS_R(2.5278963211748344e+02);

/*--------------------------------------------------------------------
c  Reference values of RMS-norms of solution error.
c-------------------------------------------------------------------*/
      xceref[0] = NAS_R(4.2348416040525025e+00);
      xceref[1] = NAS_R(4.4390282496995698e-01);
      xceref[2] = NAS_R(9.6692480136345650e-01);
      xceref[3] = NAS_R(8.8302063039765474e-01);
      xceref[4] = NAS_R(9.7379901770829278e+00);
...
c  Reference values of RMS-norms of residual.
c-------------------------------------------------------------------*/
      xcrref[0] = NAS_R(1.4233597229287254e+03);
      xcrref[1] = NAS_R(9.9330522590150238e+01);
      xcrref[2] = NAS_R(3.5646025644535285e+02);
      xcrref[3] = NAS_R(3.2485447959084092e+02);
      xcrref[4] = NAS_R(3.2707541254659363e+03);

/*--------------------------------------------------------------------
c  Reference values of RMS-norms of solution error.
c-------------------------------------------------------------------*/
      xceref[0] = NAS_R(5.2969847140936856e+01);
      xceref[1] = NAS_R(4.4632896115670668e+00);
      xceref[2] = NAS_R(1.3122573342210174e+01);
      xceref[3] = NAS_R(1.2006925323559144e+01);
      xceref[4] = NAS_R(1.2459576151035986e+02);
...
c  Reference values of RMS-norms of residual.
c-------------------------------------------------------------------*/
      xcrref[0] = NAS_R(0.62398116551764615e+04);
      xcrref[1] = NAS_R(0.50793239190423964e+03);
      xcrref[2] = NAS_R(0.15423530093013596e+04);
      xcrref[3] = NAS_R(0.13302387929291190e+04);
      xcrref[4] = NAS_R(0.11604087428436455e+05);

/*--------------------------------------------------------------------
c  Reference values of RMS-norms of solution error.
c-------------------------------------------------------------------*/
      xceref[0] = NAS_R(0.16462008369091265e+03);
      xceref[1] = NAS_R(0.11497107903824313e+02);
      xceref[2] = NAS_R(0.41207446207461508e+02);
      xceref[3] = NAS_R(0.37087651059694167e+02);
      xceref[4] = NAS_R(0.36211053051841265e+03);
...
c-------------------------------------------------------------------*/
  for (m = 0; m < 5; m++) {
    ...
    xcrdif[m] = nas_fabs((xcr[m]-xcrref[m])/xcrref[m]);
    xcedif[m] = nas_fabs((xce[m]-xceref[m])/xceref[m]);
...
c-------------------------------------------------------------------*/
    ...
    if (*class == 'U') {
      ...
      *verified = FALSE;
      printf(" FAILURE: %2d%20.13e%20.13e%20.13e\n",
	     m, NAS_PR(xcr[m]), NAS_PR(xcrref[m]), NAS_PR(xcrdif[m]));
    } else {
  ...
  for (m = 0; m < 5; m++) {
    if (*class == 'U') {
      ...
      *verified = FALSE;
      printf(" FAILURE: %2d%20.13e%20.13e%20.13e\n",
	     m, NAS_PR(xce[m]), NAS_PR(xceref[m]), NAS_PR(xcedif[m]));
    } else {
      printf("          %2d%20.13e%20.13e%20.13e\n",
	     m, NAS_PR(xce[m]), NAS_PR(xceref[m]), NAS_PR(xcedif[m]));
    }
...
--------------------------------------------------------------------*/
  ...
  nas_real pivot, coeff;

/*--------------------------------------------------------------------
c     
c-------------------------------------------------------------------*/

  pivot = NAS_R(1.00)/lhs[0][0];
...
/* ================= timer + results (own precision) ============= */
#include <sys/time.h>
static TIMET _wtime(void){struct timeval tv;static long sec=-1;gettimeofday(&tv,(void*)0);if(sec<0)sec=tv.tv_sec;return (TIMET)(tv.tv_sec-sec)+(TIMET)NAS_R(1.0e-6)*(TIMET)tv.tv_usec;}
static TIMET _start[64],_elapsed[64];
void timer_clear(int n){_elapsed[n]=(TIMET)0;}
void timer_start(int n){_start[n]=_wtime();}
void timer_stop(int n){TIMET now=_wtime();_elapsed[n]+=now-_start[n];}
TIMET timer_read(int n){return _elapsed[n];}
void c_print_results(char*name,char cls,int n1,int n2,int n3,int niter,int nthreads,TIMET t,TIMET mops,char*optype,int passed,char*ver,char*ctime,char*cc,char*clink,char*clib,char*cinc,char*cflags,char*clflags,char*rand){
    printf("\n\n %s Benchmark Completed\n",name);
    printf(" Class           =             %12c\n",cls);
    printf(" Size            =             %4dx%4dx%4d\n",n1,n2,n3);
    printf(" Iterations      =             %12d\n",niter);
    printf(" Time in seconds =             %12.2f\n",NAS_PR((nas_real)t));
    printf(" Mop/s total     =             %12.2f\n",NAS_PR((nas_real)mops));
    printf(" Operation type  = %24s\n",optype);
    printf(" Verification    =             %12s\n",passed?"SUCCESSFUL":"UNSUCCESSFUL");
