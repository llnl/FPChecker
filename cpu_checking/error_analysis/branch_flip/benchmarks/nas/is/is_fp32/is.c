...
/* Prototype was missing: is.c called this with no declaration in
   scope, so C99 synthesized an implicit variadic int(...) and clang
   emitted a bitcast of the real void(...) definition. The resulting
   ABI mismatch corrupts the two double arguments. Declaration only --
   no arithmetic and no branch changes. */
void c_print_results(char *name, char class, int n1, int n2, int n3,
                     int niter, int nthreads, double t, double mops,
                     char *optype, int passed_verification,
...
/* clang 16+ rejects K&R implicit int and implicit function declarations
   (both removed in C99).  IS has no npb-C.h, so these prototypes live here.
   Neither issue is precision-related -- IS simply never built under clang. */
extern void   timer_clear(int);
extern void   timer_start(int);
extern void   timer_stop(int);
extern double timer_read(int);
extern void   c_print_results(char *, char, int, int, int, int, int,
