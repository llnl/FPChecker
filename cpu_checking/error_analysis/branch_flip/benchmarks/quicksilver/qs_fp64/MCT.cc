...
namespace
{
   ...
   HOST_DEVICE_CUDA
   qs_real MCT_Cell_Volume_3D_G_vector_tetDet(const MC_Vector &v0_,
                                             const MC_Vector &v1_,
                                             const MC_Vector &v2_,
                                             const MC_Vector &v3);
   ...
   void MCT_Nearest_Facet_3D_G_Move_Particle(
      ...
      qs_real move_factor);      // input: multiplication factor for move
   ...
   MC_Nearest_Facet MCT_Nearest_Facet_Find_Nearest(
      ...
      qs_real &move_factor, // input/output
      int num_facets_per_cell,
      MC_Distance_To_Facet *distance_to_facet,
      int &retry /* output */ );
   ...
   HOST_DEVICE_CUDA
   qs_real MCT_Nearest_Facet_3D_G_Distance_To_Segment(
      qs_real plane_tolerance,
      qs_real facet_normal_dot_direction_cosine,
      qs_real A, qs_real B, qs_real C, qs_real D,
...
MC_Nearest_Facet MCT_Nearest_Facet(MC_Particle *mc_particle,
                                   ...
                                   MC_Vector &coordinate,
                                   const DirectionCosine *direction_cosine,
                                   qs_real distance_threshold,
                                   qs_real current_best_distance,
                                   bool new_segment,
                                  ...
                                  MonteCarlo* monteCarlo )
{
    ...
    qs_real random_number = rngSample(random_number_seed);
    qs_real which_volume = random_number * qs_real(6.0) * domain.cell_state[cell]._volume;

    // Find the tet to sample from.
    qs_real current_volume = qs_real(0.0);
    int facet_index = -1;
    ...
    while (current_volume < which_volume)
    {
        ...
        qs_real subvolume = MCT_Cell_Volume_3D_G_vector_tetDet(*point0, *point1, *point2, center);
        current_volume += subvolume;

    }

    // Sample from the tet.
    qs_real r1 = rngSample(random_number_seed);
    qs_real r2 = rngSample(random_number_seed);
    qs_real r3 = rngSample(random_number_seed);

    // Cut and fold cube into prism.
    if (r1 + r2 > qs_real(1.0))
    {
        r1 = qs_real(1.0) - r1;
        r2 = qs_real(1.0) - r2;
    }
    // Cut and fold prism into tetrahedron.
    if (r2 + r3 > qs_real(1.0))
    {
        qs_real tmp = r3;
        r3 = qs_real(1.0) - r1 - r2;
        r2 = qs_real(1.0) - tmp;
    }
    else if (r1 + r2 + r3 > qs_real(1.0))
    {
        qs_real tmp = r3;
        r3 = r1 + r2 + r3 - qs_real(1.0);
        r1 = qs_real(1.0) - r2 - tmp;
    }

    // numbers 1-4 are the barycentric coordinates of the random point.
    qs_real r4 = qs_real(1.0) - r1 - r2 - r3;
                                 ...
                                 int cell_index)
{
   ...
   qs_real one_over_num_points = qs_real(1.0)/((qs_real)num_points);
...
namespace
{
   ...
   ///  mesh.
   HOST_DEVICE_CUDA
   qs_real MCT_Nearest_Facet_3D_G_Distance_To_Segment(qs_real plane_tolerance,
                                                     qs_real facet_normal_dot_direction_cosine,
                                                     qs_real A, qs_real B, qs_real C, qs_real D,
                                                     const MC_Vector &facet_coords0,
                                                     const MC_Vector &facet_coords1,
                                                     ...
                                                     bool allow_enter)
   {
    qs_real boundingBox_tolerance = qs_real(1e-9);
    qs_real numerator = -qs_real(1.0)*(A * coordinate.x +
                             B * coordinate.y +
                             C * coordinate.z +
                             D);
    ...
    if (!allow_enter && numerator < qs_real(0.0) && numerator * numerator > plane_tolerance) {
        return PhysicalConstants::_hugeDouble; }

    // we have to restrict the solution to within the triangular face
    qs_real distance = numerator / facet_normal_dot_direction_cosine;
...
#define IF_POINT_ABOVE_CONTINUE(axis)                                    \
    ...
    // A^2 + B^2 + C^2 = 1, so max(|A|,|B|,|C|) >= 1/qsm::sqrt_(3) = 0.577
    // (all coefficients can't be small)
    qs_real cross0 = 0, cross1 = 0, cross2 = 0;
    if ( C < -qs_real(0.5) || C > qs_real(0.5) )
    {
...
#define AB_CROSS_AC(ax,ay,bx,by,cx,cy) ( (bx-ax)*(cy-ay) - (by-ay)*(cx-ax) )
    ...
    else if ( B < -qs_real(0.5) || B > qs_real(0.5) )
    {
       IF_POINT_BELOW_CONTINUE(x);
       IF_POINT_ABOVE_CONTINUE(x);
    ...
    else if ( A < -qs_real(0.5) || A > qs_real(0.5) )
    {
       IF_POINT_BELOW_CONTINUE(z);
       IF_POINT_ABOVE_CONTINUE(z);
       IF_POINT_BELOW_CONTINUE(y);
    ...
    qs_real cross_tol = qs_real(1e-9) * MC_FABS(cross0 + cross1 + cross2);  // cross product tolerance
...
void MCT_Reflect_Particle(MonteCarlo *monteCarlo, MC_Particle &particle)
{
    ...
    qs_real dot = qs_real(2.0)*( direction_cosine->alpha * facet_normal.x +
    ...
    qs_real particle_speed = particle.velocity.Length();
...
namespace
{
                                                   ...
                                                   MC_Distance_To_Facet *distance_to_facet)
   {
      ...
      {
         if ( distance_to_facet[facet_index].distance > qs_real(0.0) )
         {
                                                   ...
                                                   int &retry /* output */ )
   {
      ...
      if ( mc_particle )
      {
         if ( (nearest_facet.distance_to_facet == PhysicalConstants::_hugeDouble && move_factor > 0) ||
              ( mc_particle->num_segments > max_allowed_segments && nearest_facet.distance_to_facet <= qs_real(0.0) ) )
         {
            ...
            iteration++;
            move_factor *= qs_real(2.0);

            if ( move_factor > qs_real(1.0e-2) )
               move_factor = qs_real(1.0e-2);
...
namespace
{
      ...
      const DirectionCosine *direction_cosine)
   {
      ...
      int                    iteration = 0;
      qs_real                 move_factor = qs_real(0.5) * PhysicalConstants::_smallDouble;
      ...
      while (true) // will break out when distance is found
      {
         ...
         // (1e-8 * Radius)^2
         qs_real plane_tolerance = qs_real(1e-16)*(coordinate.x*coordinate.x +
...
//to-do        mcco->distance_to_facet->task[my_task_num].facet[facet_index].distance = PhysicalConstants::_hugeDouble;
            ...
            if (facet_normal_dot_direction_cosine <= qs_real(0.0)) { continue; }
            ...
            qs_real t = MCT_Nearest_Facet_3D_G_Distance_To_Segment(
               plane_tolerance,
