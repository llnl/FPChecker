...
{
   // find center of cell
   MC_Vector cellCenter(qs_real(0.), qs_real(0.), qs_real(0.));
...
qs_real cellVolume(const MC_Facet_Adjacency_Cell& cell,
                  const qs_vector<MC_Vector>& node)
{
   // find center of cell
   MC_Vector cellCenter(qs_real(0.), qs_real(0.), qs_real(0.));
   ...
   for (unsigned iFacet=0; iFacet<cell.num_facets; ++iFacet)
   {
      ...
      volume += qsm::abs_(aa.Dot(bb.Cross(cc)));
   }
   volume /= qs_real(6.0);
   return volume;
}
  ...
  mesh(meshPartition, grid, ddc, getBoundaryCondition(params))
{
   ...
   for (unsigned ii=0; ii<cell_state.size(); ++ii)
   {
      ...
      for (unsigned jj=0; jj<numEnergyGroups; ++jj)
         cell_state[ii]._total[jj] = qs_real(0.);
      ...
      cell_state[ii]._cellNumberDensity = qs_real(1.0);
