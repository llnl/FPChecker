...
namespace
{
   qs_real Get_Speed_From_Energy(qs_real energy);
}
...
void MC_SourceNow(MonteCarlo *monteCarlo)
{
    ...
    std::vector<qs_real> source_rate(monteCarlo->_materialDatabase->_mat.size());  // Get this from user input
    ...
    for ( int material_index = 0; material_index < monteCarlo->_materialDatabase->_mat.size(); material_index++ )
    {
        ...
        qs_real sourceRate = monteCarlo->_params.materialParams[name].sourceRate;
        source_rate[material_index] = sourceRate;
    }

    qs_real local_weight_particles = 0;
    ...
    for ( int domain_index = 0; domain_index < monteCarlo->domain.size(); domain_index++ )
    {
        ...
        for ( int cell_index = 0; cell_index < domain.cell_state.size(); cell_index++ )
        {
            ...
            qs_real cell_weight_particles = cell._volume * source_rate[cell._material] * monteCarlo->time_info->time_step;
            local_weight_particles += cell_weight_particles;
        }
    }

    qs_real total_weight_particles = 0;
    ...
    qs_real source_fraction = qs_real(0.1);
    qs_real source_particle_weight = total_weight_particles/(source_fraction * num_particles);
    ...
    for ( int domain_index = 0; domain_index < monteCarlo->domain.size(); domain_index++ )
    {
        ...
        for ( int cell_index = 0; cell_index < domain.cell_state.size(); cell_index++ )
        {
            MC_Cell_State &cell = domain.cell_state[cell_index];
            qs_real cell_weight_particles = cell._volume * source_rate[cell._material] * monteCarlo->time_info->time_step;
            qs_real cell_num_particles_float = cell_weight_particles / source_particle_weight;
		...
		QS::atomicCaptureAdd( cell._sourceTally, UINT64_C(1), random_number_seed );
                ...
                qs_real speed = Get_Speed_From_Energy(particle.kinetic_energy);
...
namespace
{
   qs_real Get_Speed_From_Energy(qs_real energy)
   {
      static const qs_real rest_mass_energy = PhysicalConstants::_neutronRestMassEnergy;
      static const qs_real speed_of_light  = PhysicalConstants::_speedOfLight;


      return speed_of_light * qsm::sqrt_(energy * (energy + qs_real(2.0)*(rest_mass_energy)) /
