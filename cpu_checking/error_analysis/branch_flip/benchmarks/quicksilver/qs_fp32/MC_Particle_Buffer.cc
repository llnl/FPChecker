...
void particle_buffer_base_type::Allocate(int buffer_size)
{
    ...
    uint64_t length_float_data = (MC_Base_Particle::num_base_floats * buffer_size    ) * (int)sizeof(qs_real);
    ...
    if ( length_int_data % sizeof(qs_real) != 0 )
    {
    ...
    this->int_data   = (int *)p;
    this->float_data = (qs_real *)(p + length_int_data);
...
void particle_buffer_base_type::Reset_Offsets()
{
    ...
    uint64_t length_float_data = (MC_Base_Particle::num_base_floats * num_particles    ) * (int)sizeof(qs_real);
