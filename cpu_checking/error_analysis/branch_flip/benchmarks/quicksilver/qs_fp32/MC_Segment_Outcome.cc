...
HOST_DEVICE
static inline unsigned int MC_Find_Min(const qs_real *array,
                                       int     num_elements);
HOST_DEVICE_END
...
MC_Segment_Outcome_type::Enum MC_Segment_Outcome(MonteCarlo* monteCarlo, MC_Particle &mc_particle, unsigned int &flux_tally_index)
{
    ...
    qs_real distance[3];
    distance[0] = distance[1] = distance[2] = qs_real(1e80);

    // Calculate the particle speed
    qs_real particle_speed = mc_particle.Get_Velocity()->Length();

    // Force collision if a census event narrowly preempts a collision
    int force_collision = 0 ;
    if ( mc_particle.num_mean_free_paths < qs_real(0.0) )
    {
        force_collision = 1 ;

        if ( mc_particle.num_mean_free_paths > -qs_real(900.0) )
        {
#if 1
    ...
    qs_real macroscopic_total_cross_section = weightedMacroscopicCrossSection(monteCarlo, 0,
    ...
    if (macroscopic_total_cross_section == qs_real(0.0))
    {
        mc_particle.mean_free_path = PhysicalConstants::_hugeDouble;
    }
    else
    {
        mc_particle.mean_free_path = qs_real(1.0) / macroscopic_total_cross_section;
    }
    ...
    if ( mc_particle.num_mean_free_paths == qs_real(0.0))
    {
        ...
        qs_real random_number = rngSample(&mc_particle.random_number_seed);

        mc_particle.num_mean_free_paths = -qs_real(1.0)*qsm::log_(random_number);
    }
    ...
    qs_real distance_threshold = qs_real(10.0) * PhysicalConstants::_hugeDouble;
    // Get the current winning distance.
    qs_real current_best_distance = PhysicalConstants::_hugeDouble;
    ...
    {
        mc_particle.num_mean_free_paths = qs_real(0.0);
    }
    ...
    qs_real segment_path_time = (mc_particle.segment_path_length/particle_speed);
    ...
    {
        mc_particle.time_to_census = qs_real(0.0);
    }
