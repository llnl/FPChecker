...
class MC_Particle
{
 public:
    ...
    qs_real num_mean_free_paths;

    // distance to a collision
    qs_real mean_free_path;

    // the distance this particle travels in a segment.
    qs_real segment_path_length;
...
public:
   ...
   HOST_DEVICE_CUDA
   void Move_Particle(const DirectionCosine & direction_cosine, const qs_real distance);
...
{
    printf( "coordiante:          %g\t%g\t%g\n", qsm::pr(coordinate.x), qsm::pr(coordinate.y), qsm::pr(coordinate.z));
    printf( "velocity:            %g\t%g\t%g\n", qsm::pr(velocity.x), qsm::pr(velocity.y), qsm::pr(velocity.z));
    printf( "direction_cosine:    %g\t%g\t%g\n", qsm::pr(direction_cosine.alpha), qsm::pr(direction_cosine.beta), qsm::pr(direction_cosine.gamma));
    printf( "kinetic_energy:      %g\n", qsm::pr(kinetic_energy));
    printf( "Weight:              %g\n", qsm::pr(weight));
    printf( "time_to_census:      %g\n", qsm::pr(time_to_census));
    printf( "totalCrossSection:   %g\n", qsm::pr(totalCrossSection));
    printf( "age:                 %g\n", qsm::pr(age));
    printf( "num_mean_free_paths: %g\n", qsm::pr(num_mean_free_paths));
    printf( "mean_free_path:      %g\n", qsm::pr(mean_free_path));
    printf( "segment_path_length: %g\n", qsm::pr(segment_path_length));
