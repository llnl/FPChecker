...
namespace
{
   ...
   void initializeCentersGrid(qs_real lx, qs_real ly, qs_real lz,
...
namespace
{
   void initNuclearData(MonteCarlo* monteCarlo, const Parameters& params)
   {
          ...
          matIter != params.materialParams.end(); matIter++)
     {
        ...
        qs_real nuBar = params.crossSectionParams.at(mp.fissionCrossSection).nuBar;
        ...
        for (int iIso=0; iIso<mp.nIsotopes; ++iIso)
        {
           ...
           material.addIsotope(Isotope(isotopeGid, qs_real(1.0)/mp.nIsotopes));
        }
...
namespace
{
   void initMesh(MonteCarlo* monteCarlo, const Parameters& params)
   {
      ...
      qs_real lx = params.simulationParams.lx;
      qs_real ly = params.simulationParams.ly;
      qs_real lz = params.simulationParams.lz;
      ...
      for (int ix=0; ix<xDom; ++ix)
         for (int iy=0; iy<yDom; ++iy)
            for (int iz=0; iz<zDom; ++iz)
               centers.push_back(
                  MC_Vector( (qs_real(0.5)+ix)*dx, (qs_real(0.5)+iy)*dy, (qs_real(0.5)+iz)*dz )
               );
   }
}
...
namespace
{
   ...
   void checkCrossSections(MonteCarlo* monteCarlo, const Parameters& params)
   {
      ...
      struct XC_Data
      {
         XC_Data() : absorption(qs_real(0.)), fission(qs_real(0.)), scatter(qs_real(0.)){}
         qs_real absorption;
         qs_real fission;
         qs_real scatter;
      };
      ...
      vector<qs_real> energy(nGroups);
      for (unsigned ii=0; ii<nGroups; ++ii)
         energy[ii] = (nd->_energies[ii] + nd->_energies[ii+1])/qs_real(2.0);
    ...
    xSec = fopen( fileName.c_str(), "w" );
      ...
      for (unsigned ii=0; ii<nGroups; ++ii)
      {
         fprintf(xSec, "%u  %g", ii, qsm::pr(energy[ii]));
         for (auto mapIter=xcTable.begin(); mapIter!=xcTable.end(); ++mapIter)
         {
            fprintf(xSec, "  %g  %g  %g", qsm::pr(mapIter->second[ii].absorption), qsm::pr(mapIter->second[ii].fission), qsm::pr(mapIter->second[ii].scatter));
         }
         fprintf(xSec, "\n");
      }
    fclose( xSec );
