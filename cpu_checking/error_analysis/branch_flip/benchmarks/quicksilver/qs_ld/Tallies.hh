...
class ScalarFluxCell
{
   ...
   ScalarFluxCell(qs_real* storage, int size)
   :  _group(storage),
      _size(size)
   {
      for( int i = 0; i < _size; i++ )
      {
         _group[i] = qs_real(0.0);
      }
   }
...
class ScalarFluxTask
{
   ...
   BulkStorage<qs_real> _scalarFluxCellStorage;
   ...
   ScalarFluxTask(MC_Domain* domain, int numGroups)
   {
      ...
      for (int cellIndex = 0; cellIndex < domain->cell_state.size(); cellIndex++)
      {
         qs_real* tmp = _scalarFluxCellStorage.getBlock(numGroups);
   ...
   void Reset() 
   {
      ...
      for (int cellIndex = 0; cellIndex < _cell.size(); cellIndex++)
      {
         for (int groupIndex = 0; groupIndex < numGroups; groupIndex++)
         {
            _cell[cellIndex]._group[groupIndex] = qs_real(0.0);
         }
      }     
   }
...
{
  public:
    FluenceDomain( int numCells) : _cell(numCells, qs_real(0.0))
    {}

    void addCell( int index, qs_real value ){ _cell[index] += value;}
    qs_real getCell( int index ){ return _cell[index]; }
    int size(){ return _cell.size(); }
    
  private:
    std::vector<qs_real> _cell;
};
...
class Tallies
{
  public:
    ...
    HOST_DEVICE_CUDA
    void TallyScalarFlux(qs_real value, int domain, int task, int cell, int group)
    {
        QS::atomicAdd( _scalarFluxDomain[domain]._task[task]._cell[cell]._group[group], value );
    }

    HOST_DEVICE_CUDA
    void TallyCellValue(qs_real value, int domain, int task, int cell)
    {
        QS::atomicAdd( _cellTallyDomain[domain]._task[task]._cell[cell], value );
    }

    qs_real ScalarFluxSum(MonteCarlo *mcco);
