...
class MC_Base_Particle
{
  public:
    ...
    // serialize the vault
    void Serialize(int *int_data, qs_real *float_data, char *char_data,
    ...
    HOST_DEVICE_CUDA
    inline qs_real Get_Energy() const                     { return kinetic_energy; }
    HOST_DEVICE_CUDA
...
inline MC_Base_Particle::MC_Base_Particle( ) :
        coordinate(),
        velocity(),
        kinetic_energy(qs_real(0.0)),
        weight(qs_real(0.0)),
        time_to_census(qs_real(0.0)),
        age(qs_real(0.0)),
        num_mean_free_paths(qs_real(0.0)),
        num_segments(qs_real(0.0)),
...
inline MC_Particle::MC_Particle()
   : coordinate(),
     ...
     weight(qs_real(0.0)),
     time_to_census(qs_real(0.0)),
     totalCrossSection(qs_real(0.0)),
     age(qs_real(0.0)),
     num_mean_free_paths(qs_real(0.0)),
     mean_free_path(qs_real(0.0)),
     segment_path_length(qs_real(0.0)),
     random_number_seed((uint64_t)0),
     identifier( (uint64_t)0),
     last_event(MC_Tally_Event::Census),
     num_collisions (0),
...
inline MC_Particle::MC_Particle( const MC_Base_Particle &from_particle )
   : coordinate(from_particle.coordinate),
     ...
     normal_dot(qs_real(0.0))
{
    qs_real speed = from_particle.velocity.Length();

    if ( speed > 0 )
    {
        qs_real factor = qs_real(1.0)/speed;
