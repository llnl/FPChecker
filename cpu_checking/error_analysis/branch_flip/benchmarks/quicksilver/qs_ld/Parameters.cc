...
namespace
{
   void parseCommandLine(int argc, char** argv, Parameters& pp)
   {
      ...
      addArg("dt",               'D', 1, 'r', &(sp.dt),          0,      "time step (seconds)");
      addArg("fMax",             'f', 1, 'r', &(sp.fMax),        0,      "max random mesh node displacement");
      ...
      addArg("lx",               'X', 1, 'r', &(sp.lx),          0,      "x-size of simulation (cm)");
      addArg("ly",               'Y', 1, 'r', &(sp.ly),          0,      "y-size of simulation (cm)");
      addArg("lz",               'Z', 1, 'r', &(sp.lz),          0,      "z-size of simulation (cm)");
...
namespace
{
   ...
   void supplyDefaults(Parameters& params)
   {
      ...
      sourceMaterial.mass = qs_real(1000.0);
      sourceMaterial.sourceRate = qs_real(1e10);
      ...
      sourceMaterial.fissionCrossSectionRatio = qs_real(0.1);
...
namespace
{
   void scanSimulationBlock(const InputBlock& input, Parameters& pp)
   {
      ...
      input.getValue<qs_real>("dt",          sp.dt);
      input.getValue<qs_real>("fMax",        sp.fMax);
      ...
      input.getValue<qs_real>("lx",          sp.lx);
      input.getValue<qs_real>("ly",          sp.ly);
      input.getValue<qs_real>("lz",          sp.lz);
      ...
      input.getValue<qs_real>("eMax",        sp.eMax);
      input.getValue<qs_real>("eMin",        sp.eMin);
      input.getValue<int>   ("nGroups",     sp.nGroups);
      input.getValue<qs_real>("lowWeightCutoff",sp.lowWeightCutoff);
...
namespace
{
   void scanGeometryBlock(const InputBlock& input, Parameters& pp)
   {
      ...
      if (shape == "brick")
      {
         ...
         input.getValue<qs_real>("xMax", gg.xMax);
         input.getValue<qs_real>("xMin", gg.xMin);
         input.getValue<qs_real>("yMax", gg.yMax);
         input.getValue<qs_real>("yMin", gg.yMin);
         input.getValue<qs_real>("zMax", gg.zMax);
         input.getValue<qs_real>("zMin", gg.zMin);
      }
      else if (shape == "sphere")
      {
         gg.shape = GeometryParameters::SPHERE;
         input.getValue<qs_real>("radius",  gg.radius);
         input.getValue<qs_real>("xCenter", gg.xCenter);
         input.getValue<qs_real>("yCenter", gg.yCenter);
         input.getValue<qs_real>("zCenter", gg.zCenter);
      }
      else
...
namespace
{
   void scanMaterialBlock(const InputBlock& input, Parameters& pp)
   {
      ...
      input.getValue<qs_real>("mass", mp.mass);
      input.getValue<string>("absorptionCrossSection",      mp.absorptionCrossSection);
      input.getValue<qs_real>("absorptionCrossSectionRatio", mp.absorptionCrossSectionRatio);
      input.getValue<string>("fissionCrossSection",         mp.fissionCrossSection);
      input.getValue<qs_real>("fissionCrossSectionRatio",    mp.fissionCrossSectionRatio);
      input.getValue<int>   ("nIsotopes",                   mp.nIsotopes);
      input.getValue<int>   ("nReactions",                  mp.nReactions);
      input.getValue<qs_real>("totalCrossSection",           mp.totalCrossSection);
      input.getValue<string>("scatteringCrossSection",      mp.scatteringCrossSection);
      input.getValue<qs_real>("scatteringCrossSectionRatio", mp.scatteringCrossSectionRatio);
      input.getValue<qs_real>("sourceRate",                  mp.sourceRate);
   }
}
namespace
{
   void scanCrossSectionBlock(const InputBlock& input, Parameters& pp)
   {
      ...
      input.getValue<qs_real>("A",       cp.aa);
      input.getValue<qs_real>("B",       cp.bb);
      input.getValue<qs_real>("C",       cp.cc);
      input.getValue<qs_real>("D",       cp.dd);
      input.getValue<qs_real>("E",       cp.ee);
      input.getValue<qs_real>("nuBar",   cp.nuBar);
