...
namespace
{
   void PopulationControlGuts(const qs_real splitRRFactor, 
...
void PopulationControl(MonteCarlo* monteCarlo, bool loadBalance)
{
    ...
    if (loadBalance)
    {
        ...
        targetNumParticles = qsm::ceil_((qs_real)targetNumParticles / (qs_real)mcco->processor_info->num_processors );
...
//        targetNumParticles = qsm::ceil_((qs_real)targetNumParticles / (qs_real)mcco->processor_info->num_tasks);
    }
    else
    {
    ...
    qs_real splitRRFactor = qs_real(1.0);
    if (loadBalance)
    {
        int currentNumParticles = localNumParticles;
        if (currentNumParticles != 0)
            splitRRFactor = (qs_real)targetNumParticles / (qs_real)currentNumParticles;
        else
            splitRRFactor = qs_real(1.0);
    }
    else
    {
        splitRRFactor = (qs_real)targetNumParticles / (qs_real)globalNumParticles;
    }

    if (splitRRFactor != qs_real(1.0))  // no need to split if population is already correct.
...
namespace
{
void PopulationControlGuts(const qs_real splitRRFactor, uint64_t currentNumParticles, ParticleVaultContainer* my_particle_vault, Balance& taskBalance)
{
    ...
    for (int particleIndex = currentNumParticles-1; particleIndex >= 0; particleIndex--)
    {
        ...
        qs_real randomNumber = rngSample(&currentParticle.random_number_seed);
        if (splitRRFactor < 1)
        {
        ...
        {
            // Split
	        int splitFactor = (int)qsm::floor_(splitRRFactor);
...
void RouletteLowWeightParticles(MonteCarlo* monteCarlo)
{
    ...
    const qs_real lowWeightCutoff = monteCarlo->_params.simulationParams.lowWeightCutoff;

    if (lowWeightCutoff > qs_real(0.0))
    {
	    ...
	    const qs_real source_particle_weight = monteCarlo->source_particle_weight;
	    const qs_real weightCutoff = lowWeightCutoff*source_particle_weight;
