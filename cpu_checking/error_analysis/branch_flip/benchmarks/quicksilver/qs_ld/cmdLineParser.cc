      ...
      else
      {
         switch(o->type)
         {
            ...
            case 'd':
               ...
               break;
            case 'r': {
               // Parse at full width, then store at working precision.
               // Writing "%lf" straight into a qs_real* would be an
               // 8-byte store into a 4-byte object when qs_real=float.
               double tmp_ = 0.0;
               sscanf(optarg,"%lf",&tmp_);
               *(qs_real*)o->ptr = (qs_real)tmp_;
               break; }
            case 's':
