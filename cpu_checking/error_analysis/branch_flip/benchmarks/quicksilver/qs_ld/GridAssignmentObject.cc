...
: _centers(centers)
{
   ...
   // 1.
   qs_real lx = max(qs_real(1.), (maxCoord.x - minCoord.x));
   qs_real ly = max(qs_real(1.), (maxCoord.y - minCoord.y));
   qs_real lz = max(qs_real(1.), (maxCoord.z - minCoord.z));

   qs_real x = _centers.size()/centersPerCell/(lx*ly*lz);
   x = qsm::pow_(x, qs_real(1.0)/qs_real(3.0));
   _nx = max(qs_real(1.), qsm::floor_(x*lx));
   _ny = max(qs_real(1.), qsm::floor_(x*ly));
   _nz = max(qs_real(1.), qsm::floor_(x*lz));
   _dx = lx/_nx;
   _dy = ly/_ny;
   _dz = lz/_nz;
...
qs_real GridAssignmentObject::minDist2(const MC_Vector r, int iCell) const
{
   Tuple ir = whichCellTuple(r);
   Tuple iTuple = indexToTuple(iCell);

   qs_real rx = _dx*(abs(iTuple.x() - ir.x()) - 1); rx = max(qs_real(0.), rx);
   qs_real ry = _dy*(abs(iTuple.y() - ir.y()) - 1); ry = max(qs_real(0.), ry);
   qs_real rz = _dz*(abs(iTuple.z() - ir.z()) - 1); rz = max(qs_real(0.), rz);
