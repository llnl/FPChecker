                    ...
                    Int_t *col, Int_t *row, Int_t *plane, Int_t *side)
{
   ...
   #if USE_MPI
   /* The MPI path picks its datatype from sizeof(Real_t) and has no
      long double case (see baseType in lulesh-comm.cc), so keep the
      guard there.  A serial build needs no MPI datatype at all. */
