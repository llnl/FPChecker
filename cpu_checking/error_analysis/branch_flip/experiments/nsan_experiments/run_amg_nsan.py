#!/usr/bin/env python3
"""
run_amg_nsan.py -- build AMG under nsan with the NsanBFSites plugin and run
it, fp32 and fp64 separately.

    ./run_amg_nsan.py                       # both precisions
    ./run_amg_nsan.py -p fp32 -n 10
    ./run_amg_nsan.py --sites-only
    ./run_amg_nsan.py --resume-policy discard --eq-policy truncated

Shadow mapping dqq for fp32 (fp32-vs-fp64 cell), dlq for fp64 (fp64-vs-ld).
AMG is oracle-stable at fp64, so that cell is a false-alarm measurement.

AMG-specific gates: sizeof(HYPRE_Real) probe (a clean build does not prove
-DHYPRE_SINGLE reached every TU) and iterations != 0 (the sequential MPI
stubs silently fall through at non-double precision). Census config is
problem 2, n=5; problem 1 never diverges.
...
def build(precision, opt, outdir, eq_policy, jobs=1, cc="clang"):
    ...
    cflags = (f"-g -{opt} -DHYPRE_SEQUENTIAL=1 -I../utilities {flag} "
              f"-fsanitize=numerical -fpass-plugin={PLUGIN} "
              f"-mllvm -nsan-shadow-type-mapping={mapping} "
              f"-mllvm -nsan-truncate-fcmp-eq={eq}").strip()
    lflags = f"-lm -fsanitize=numerical {SHIM} {wrap}"
    cmd = ["make", f"-j{jobs}", f"CC=clang",
    ...
    for mod, b in top:
        if b[1]:
            print(f"      {mod:<24s} mod {b[0]:<11d} {b[1]:5d} branch "
                  f"{b[2]:4d} select")
    ...
    # --- gate 4: precision flag reached the headers ---
...
def run(binary, precision, problem, n, outdir, opt, resume_policy):
    env = base_env()
    ...
    return {
        "precision": precision, "opt": opt, "problem": problem, "n": n,
...
def write_results(precision, r, outdir):
    L = [f"AMG {precision} -- nsan branch flips",
         f"(-{r['opt']} compile+link, shadow map {r['shadow_map']}, "
         f"problem {r['problem']}, n={r['n']}^3)", ""]
    L.append(f"{r['branch_flips']} branch flips @ {r['locations']} site(s)")
    L.append(f"iterations: {r['iterations']}   residual: {r['residual']}")
    L.append("")
...
def main():
    ...
    ap.add_argument("--opt", default="O0", choices=["O0", "O1", "O2", "O3"],
                    help="compile AND link level, exported as "
                         ...
                         "Site ids correspond across levels; occurrence "
                         "indices do not. MUST match the oracle's level.")
    ...
    print(f"AMG / nsan   problem {args.problem}  n={args.n}^3  -{args.opt}")
    ...
    for precision in args.precision:
        ...
        r = run(binary, precision, args.problem, args.n, outdir, args.opt,
                args.resume_policy)
        ...
        print(f"  -> {r['branch_flips']} branch flips @ {r['locations']} "
              f"site(s)  iters={r['iterations']}{extra}")
