...
def test_1():
    ...
    # --- run code ---
    cmd = ["./main \"0.123,2.123,3.123\" scaled"]
    try:
        cmdOutput = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True)
    except subprocess.CalledProcessError as e:
        print(e.output)
    ...
    long_double_total_error = float(diff_match.group(1))

    target_line = find_source_line("main.cpp", "val_sum_f += values_f[i] + result_f;")
    assert target_line != -1, "Could not find FP64 accumulation line in main.cpp"
