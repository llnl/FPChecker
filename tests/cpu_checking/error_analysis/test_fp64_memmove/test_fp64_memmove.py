...
def test_1():
    ...
    # --- run code ---
    cmd = ["./memmove_test \"0.123,2.123,3.123\""]
    try:
        cmdOutput = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True)
    except subprocess.CalledProcessError as e:
        print(e.output)
    ...
    for i in range(len(data)):
        print('i', i, data[i])
        if data[i]['file'].endswith('memmove_fp64_test.cpp'):
            if data[i]['line'] == 54:
    ...
    diff = abs(long_double_error - rounding_error_fp64)
    print("Diff =", diff)
