...
__attribute__((noinline)) void scale_double_values(const double *input, double *output, size_t size, bool use_volatile = true)
{
    ...
    { // Option B — use memcpy
        ...
        {
            tmp_values[i] = input[i] * scale;
            // printf("(MEMCPY) Scaled value %zu: %.6f\n", i, tmp_values[i]);
        }
        std::memcpy(output, tmp_values, size * sizeof(double));
    }
}

__attribute__((noinline)) void scale_double_values_long_d(const long double *input, long double *output, size_t size)
{
    ...
    std::memcpy(output, tmp_values, size * sizeof(long double));
...
int main(int argc, char *argv[])
{
    ...
    long double output_values_d[size_d];
    scale_double_values_long_d(values_ld.data(), output_values_d, size_d);

    long double result_ld = 0.0;
    // calc_dot_product_f(output_values, output_values, size, result);
    result_ld = sum_of_elements_ld(output_values_d, size_d);
    printf("Result (long double): %.21Le\n", result_ld);

    long double difference = result_ld - static_cast<long double>(result);
    printf("Difference: %.17Le\n", difference);
