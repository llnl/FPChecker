...
def parse_fpchecker_output():
    ...
    try:
        result = subprocess.check_output(["./main", "1.000", "1.002"], stderr=subprocess.STDOUT)
...
def test_fp32_roundError_cancellation():
    found = False
    cancellation_error = parse_fpchecker_output()
    print(cancellation_error)
    assert cancellation_error is not None
   
    assert abs(cancellation_error) > 1e-9, f"Cancellation error too small: {cancellation_error}"
    print("Cacellation error after the assert", abs(cancellation_error))
    found = True
