...
#endif
{
  ...
  dtfixed() = TimeReal_t(-1.0e-6) ; // Negative means use courant condition
  stoptime()  = TimeReal_t(1.0e-2); // *Real_t(edgeElems*tp/45.0) ;

   // Initial conditions
  deltatimemultlb() = TimeReal_t(1.1) ;
  deltatimemultub() = TimeReal_t(1.2) ;
  dtcourant() = TimeReal_t(1.0e+20) ;
  dthydro()   = TimeReal_t(1.0e+20) ;
  dtmax()     = TimeReal_t(1.0e-2) ;
  time()    = TimeReal_t(0.) ;
   cycle()   = Int_t(0) ;
  ...
  deltatime() = TimeReal_t((Real_t(.5)*cbrt(volo(0)))/sqrt(Real_t(2.0)*einit));
