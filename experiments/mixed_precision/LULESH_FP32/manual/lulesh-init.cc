...
#endif
{
   ...
   for (Index_t i=0; i<numElem(); ++i) {
      ...
      // volume calculations
      mixedprecision_t volume = CalcElemVolume(x_local, y_local, z_local );
      volo(i) = static_cast<double> (volume );
      elemMass(i) = volume ;
      for (Index_t j=0; j<8; ++j) {
         Index_t idx = elemToNode[j] ;
         nodalMass(idx) += mixedprecision_t(volume) / mixedprecision_t(8.0) ;
      }
   }
   ...
   mixedprecision_t scale = (nx*m_tp)/mixedprecision_t(45.0);
   mixedprecision_t einit = ebase*scale*scale*scale;
   ...
   deltatime() = (mixedprecision_t(.5)*cbrt(volo(0)))/sqrt(mixedprecision_t(2.0)*einit);
