...
static
void kernel_atax_long_double(int m, int n,
		 long double POLYBENCH_2D(A,M,N,m,n),
		 long double POLYBENCH_1D(x,N,n),
		 long double POLYBENCH_1D(y,N,n),
		 long double POLYBENCH_1D(tmp,M,m))
{
  int i, j;
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_1D_ARRAY_DECL(tmp_long_double, long double, M, m);

  /* Initialize array(s). */
  init_array (m, n, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(x));
  init_array_long_double (m, n, POLYBENCH_ARRAY(A_long_double), POLYBENCH_ARRAY(x_long_double));
