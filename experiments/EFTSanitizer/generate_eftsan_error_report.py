...
def compare_benchmark(
    ...
    items, diagnostics = load_eftsan_json(json_file)
    if diagnostics != "-":
        return [
            EftsanEntry(
                benchmark=benchmark,
    ...
    for item in items:
        ...
        entries.append(
            EftsanEntry(
                ...
                line=line,
                error=error,
                status="OK" if error is not None else "BAD_ERROR_VALUE",
                diagnostics="-",
            )
        )
    return entries
...
def build_rows(entries: Sequence[EftsanEntry]) -> Tuple[List[str], List[List[str]]]:
    headers = ["Benchmark", "Source", "Line", "EFTSanError", "Diagnostics", "Status"]
    rows = []
