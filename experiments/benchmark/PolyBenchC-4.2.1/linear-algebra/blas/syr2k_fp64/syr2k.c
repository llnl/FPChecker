...
void init_array_long_double(int n, int m,
		...
		long double POLYBENCH_2D(B,N,M,n,m))
{
  int i, j;
  ...
  *beta = 1.2L;
  for (i = 0; i < n; i++)
    for (j = 0; j < m; j++) {
      ...
      B[i][j] = (long double) ((i*j+2)%m) / m;
    }
  for (i = 0; i < n; i++)
    for (j = 0; j < n; j++) {
      C[i][j] = (long double) ((i*j+3)%n) / m;
    }
}
...
static
void kernel_syr2k_long_double(int n, int m,
		  long double alpha,
		  long double beta,
		  long double POLYBENCH_2D(C,N,N,n,n),
		  long double POLYBENCH_2D(A,N,M,n,m),
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_2D_ARRAY_DECL(B_long_double,long double,N,M,n,m);
  ...
  init_array_long_double (n, m, &alpha_long_double, &beta_long_double,
